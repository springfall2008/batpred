...
def improved_car_subtraction(load_cumulative, car_cumulative, step=5, interpolate_gaps=True, max_gap_minutes=60, smoothing_window=3, debug=False):
    """
...
if load_minutes_orig and car_charging_energy:
    ...
    improved_result = improved_car_subtraction(load_minutes_orig, car_charging_energy, interpolate_gaps=True, max_gap_minutes=60, smoothing_window=5, debug=False)
...
if load_minutes_orig:
    ax1.plot(hours, [load_minutes_orig[m] for m in minutes_sorted], "b-", linewidth=1.5, label="Original Load (before car subtraction)", alpha=0.7)

ax1.plot(hours, [load_minutes[m] for m in minutes_sorted], "g-", linewidth=1.5, label="Processed Load (after car subtraction)", alpha=0.9)

if car_charging_energy:
    car_minutes_sorted = sorted(car_charging_energy.keys())
    car_hours = [-m / 60 for m in car_minutes_sorted]
    ax1.plot(car_hours, [car_charging_energy[m] for m in car_minutes_sorted], "r-", linewidth=1.0, label="Car Charging Energy", alpha=0.6)
...
if load_orig_energy:
    ...
    ax2.plot(load_orig_hours, [load_orig_energy[m] for m in load_orig_sorted], "b-", linewidth=1.0, label="Original Load (before subtraction)", alpha=0.4)

if car_energy:
    car_sorted = sorted(car_energy.keys())
    car_hours = [-m / 60 for m in car_sorted]
    ax2.plot(car_hours, [car_energy[m] for m in car_sorted], "r-", linewidth=1.0, label="Car Charging", alpha=0.5)

if load_energy:
    load_sorted = sorted(load_energy.keys())
    load_hours = [-m / 60 for m in load_sorted]
    ax2.plot(load_hours, [load_energy[m] for m in load_sorted], "orange", linewidth=1.2, label="Current Predbat Result", alpha=0.7, linestyle="--")

if simple_energy:
    simple_sorted = sorted(simple_energy.keys())
    simple_hours = [-m / 60 for m in simple_sorted]
    ax2.plot(simple_hours, [simple_energy[m] for m in simple_sorted], "purple", linewidth=1.2, label="Simple Subtraction", alpha=0.8, linestyle="-.")

if improved_energy and 0:
    improved_sorted = sorted(improved_energy.keys())
    improved_hours = [-m / 60 for m in improved_sorted]
    ax2.plot(improved_hours, [improved_energy[m] for m in improved_sorted], "g-", linewidth=1.5, label="Improved Subtraction", alpha=0.9)
...
if ax3 and simple_energy and improved_energy:
    ...
    if load_energy:
        ...
        ax3.plot(current_diff_hours, current_diff_values, "orange", linewidth=1.2, label="Improved - Current Predbat", alpha=0.7)

    ax3.plot(diff_hours, diff_values, "purple", linewidth=1.5, label="Improved - Simple", alpha=0.9)
    ...
    ax3.text(0.02, 0.98, stats_text, transform=ax3.transAxes, fontsize=10, verticalalignment="top", bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.5))
