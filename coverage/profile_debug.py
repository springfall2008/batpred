...
def profile_debug_case(debug_file, output_file=None, view=False, sort_by="cumulative", top_n=30):
    ...
    cmd = [
        "python3", "-m", "cProfile", "-o", output_file,
        unit_test_path, "--debug_file", debug_file
    ]
...
Examples:
  ...
  python3 profile_debug.py cases/predbat_debug_agile1.yaml -o custom.prof -s time -n 50
    ...
    parser.add_argument(
        "debug_file",
        help="Path to debug YAML file (e.g., cases/predbat_debug_agile1.yaml)"
    )
    
    parser.add_argument(
        "-o", "--output",
        help="Output file for profile data (default: auto-generated from input file)"
    )
    
    parser.add_argument(
        "-v", "--view",
        action="store_true",
        help="Automatically open profile in snakeviz after completion"
    )
    
    parser.add_argument(
        "-s", "--sort",
        default="cumulative",
        choices=["cumulative", "time", "calls", "ncalls"],
        help="Sort statistics by this key (default: cumulative)"
    )
