...
| Plan seems stuck / doesn't adopt a clearly better replan | `should_replace_plan()` (`plan.py:1087`) compares pre-clip snapshots; a retained incumbent keeps its stale snapshot without the expired-window pruning `charge_window_best` gets, so the improvement gap can read identical for hours while real costs move. Combined with the fragmentation tie-break (`plan.py:1098`), which always favours the plan that adds no slot, a genuinely better plan can stay rejected until something else invalidates the plan outright. Confirmed from a 24h log: 180 consecutive comparisons with an unchanged gap, only 7 of 187 total ever adopting a new plan (GH#4736). Not a bug in the tie-break itself — `test_plan_tiebreak.py` asserts exactly that rule on purpose. |
