...
class Inverter:
    ...
    def __init__(self, base, id=0, quiet=False):
        ...
        if self.inverter_time:
            ...
            if not quiet:
                self.base.log(
                    "Invertor time {} AppDeamon time {} difference {} minutes".format(
                        self.inverter_time, self.base.now_utc, tdiff
                    )
                )
            if abs(tdiff) >= 5:
                self.base.log(
                    "WARN: Invertor time is {} AppDeamon time {} this is {} minutes skewed, Predbat may not function correctly, please fix this by updating your inverter or fixing AppDeamon time zone".format(
                        self.inverter_time, self.base.now_utc, tdiff
                    )
                )
                self.base.record_status(
                    "Invertor time is {} AppDeamon time {} this is {} minutes skewed, Predbat may not function correctly, please fix this by updating your inverter or fixing AppDeamon time zone".format(
                        self.inverter_time, self.base.now_utc, tdiff
                    ),
                    had_errors=True,
                )
    ...
    def update_status(self, minutes_now, quiet=False):
        ...
        if not quiet:
            self.base.log(
                "Inverter {} SOC: {} kw {} % Current charge rate {} w Current discharge rate {} wcurrent power {} w".format(
                    self.id,
    ...
    def adjust_charge_rate(self, new_rate, notify=True):
        ...
        if abs(current_rate - new_rate) > 100:
            self.base.log(
                "Inverter {} current charge rate is {} and new target is {}".format(self.id, current_rate, new_rate)
            )
            if SIMULATE:
    ...
    def adjust_discharge_rate(self, new_rate, notify=True):
        ...
        if abs(current_rate - new_rate) > 100:
            self.base.log(
                "Inverter {} current discharge rate is {} and new target is {}".format(self.id, current_rate, new_rate)
            )
            if SIMULATE:
        ...
        if self.inverter_type == "GS":
            ...
            for retry in range(0, 6):
                ...
                if abs(old_value - new_value) <= fuzzy:
                    self.base.log(
                        "Inverter {} Wrote {} to {}, successfully now {}".format(
                            self.id, name, new_value, int(entity.get_state())
                        )
                    )
                    return True
            self.base.log(
    ...
    def rest_setChargeRate(self, rate):
        ...
        for retry in range(0, 5):
            ...
            if abs(new - rate) < 100:
                self.base.log(
                    "Inverter {} set charge rate {} via REST succesfull on retry {}".format(self.id, rate, retry)
                )
                return True
    ...
    def rest_setDischargeRate(self, rate):
        ...
        for retry in range(0, 5):
            ...
            if abs(new - rate) < 100:
                self.base.log(
                    "Inverter {} set discharge rate {} via REST succesfull on retry {}".format(self.id, rate, retry)
                )
                return True
    ...
    def rest_setReserve(self, target):
        ...
        self.base.log(
            "WARN: Set inverter {} reserve {} via REST failed on retry {} got {}".format(self.id, target, retry, result)
        )
        self.base.record_status(
            "Warn - Inverter {} REST failed to setReserve to {} got {}".format(self.id, target, result), had_errors=True
        )
        return False
...
class PredBat(hass.Hass):
    ...
    def step_data_history(
        self, item, minutes_now, forward, step=PREDICT_STEP, scale_today=1.0, car_filter=False, load_forecast={}
    ):
        """
    ...
    def calc_percent_limit(self, charge_limit):
        ...
        """
        return [
            min(int((float(charge_limit[i]) / self.soc_max * 100.0) + 0.5), 100) for i in range(0, len(charge_limit))
        ]
    ...
    def run_prediction(
        ...
        if save and save == "best" and not SIMULATE:
            ...
            self.set_state(
                self.prefix + ".record",
                state=0.0,
                attributes={"results": record_time, "friendly_name": "Prediction window", "state_class": "measurement"},
            )
            self.set_state(
        ...
        for loop_price in all_prices:
            ...
            for discharge_enable in discharge_enable_options:
                ...
                if try_charge_limit == best_limits and best_discharge == try_discharge and best_metric != 9999999:
                    self.log(
                        "Skip this optimisation as it's the same charge {} discharge {}".format(
                            try_charge_limit, try_discharge
                        )
                    )
                    continue
        ...
        for loop_limit in loop_options:
            ...
            while loop_start >= window["start"]:
                ...
                this_discharge_limit = max(
                    max(self.best_soc_min, self.reserve) * 100.0 / self.soc_max, this_discharge_limit
                )
    ...
    def discard_unused_charge_slots(self, charge_limit_best, charge_window_best, reserve):
        ...
        for window_n in range(0, max_slots):
            ...
            if (charge_limit_best[window_n] > self.dp2(reserve)) or (
                ...
                if (count % 16) == 0:
                    self.log(
                        "Final optimisation type {} window {} metric {} metric_keep {} cost {}".format(
                            typ, window_n, best_metric, self.dp2(metric_keep), self.dp2(best_cost)
                        )
                    )
                count += 1
            self.log(
                "Second pass optimisation finished metric {} cost {} metric_keep {}".format(
                    best_metric, self.dp2(best_cost), self.dp2(metric_keep)
                )
            )
    ...
    def calculate_plan(self, recompute=True):
        ...
        if recompute and self.calculate_best:
            ...
            self.optimise_all_windows(
                end_record, load_minutes_step, pv_forecast_minute_step, pv_forecast_minute10_step, metric, metric_keep
            )
            ...
            if self.calculate_best_charge and self.charge_window_best:
                ...
                record_charge_windows = max(
                    self.max_charge_windows(end_record + self.minutes_now, self.charge_window_best), 1
                )
    ...
    def update_pred(self, scheduled=True):
        ...
        if not scheduled or not self.plan_valid:
            ...
            self.log(
                "Plan was last updated on {} and is now {} minutes old".format(self.plan_last_updated, plan_age_minutes)
            )
            ...
            if (plan_age_minutes + RUN_EVERY) > self.calculate_plan_every:
                self.log(
                    "Will recompute the plan as it is now {} minutes old and will exceed the max age of {} before the next run".format(
                        plan_age_minutes, self.calculate_plan_every
                    )
                )
    ...
    def switch_event(self, event, data, kwargs):
        ...
        # Add additional config for Solis inverters
        items = CONFIG_ITEMS
        if self.inverter_type == "GS":
            items = items + CONFIG_ITEMS_SOLIS
    ...
    def initialize(self):
        ...
        if self.get_arg("template", False):
            self.log(
                "ERROR: You still have a template configuration, please edit apps.yaml or restart AppDeamon if you just updated with HACS"
            )
            self.record_status(
                "ERROR: You still have a template configuration, please edit apps.yaml or restart AppDeamon if you just updated with HACS"
            )
