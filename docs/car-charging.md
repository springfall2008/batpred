...
If car_charging_now is not set, there is currently **no other reliable way for Predbat to confirm a daytime dispatch slot in real time** - this is true regardless of your car_energy_reported_load setting. Predbat's other load-based heuristics (e.g. dynamic_load's car-slot cancellation) are derived from the same provisional Octopus dispatch data being confirmed, so they cannot independently verify it, and Octopus's own "completed dispatch" confirmation may not arrive until after the slot has already ended. In practice, if you're on a tariff with this dynamic daytime-slot scheme and want the discharge-hold/rescission protection to actually do anything for daytime slots, configuring car_charging_now is effectively required, not optional. Predbat will log a warning at startup if switch.predbat_octopus_intelligent_confirm_slots is On, Octopus Intelligent Charging is enabled, and car_charging_now is not set.
