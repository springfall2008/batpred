...
- You can configure **car_charging_now** in `apps.yaml` to point a Home Assistant sensor that indicates that the car is currently charging as a workaround to indicate your car is charging, but the Intelligent API hasn't reported it. This is also the sensor **select.predbat_trust_future_dynamic_iog_slots**'s `started` level needs - see below.
...
Note that a slot's start time passing is not itself confirmation - Octopus can still revoke a dispatch that has technically started but where the car never actually drew power, so none of these levels trust a slot on clock time alone:
    ...
    - `none` - only the fixed 23:30-05:30 window is trusted as cheap for battery planning. Every daytime dispatch is left at the normal rate, including one Octopus has already reported as completed - this is the strictest setting, and the house battery never plans around a dynamic slot at all. (Your actual costs are still reported against what really happened - see below.)
      There is deliberately no level that waits for Octopus's completed (metered) record alone: that record only appears once a slot has already ended, and can lag it by up to an hour, so it could never arrive in time to influence any planning decision about the slot it confirms. `started` does still accept it where it is available, since it is stronger evidence than a live sensor reading.
    - `started` - trusts a "trusted streak" of settlement blocks of a still-provisional dispatch, started the moment **car_charging_now** has reported the car actually charging at some point during a block (with a few minutes of that block still left - a reading seen only in a block's closing minute or two does not start a streak, since that's ambiguous trailing/residual draw rather than confirmation of a fresh block). Once started, the streak rolls forward and keeps trusting each following block automatically, without needing its own fresh reading, right up until Predbat sees an explicit "not charging" reading - real charger/readiness sensors have been observed briefly toggling right at a settlement-block boundary (negotiation noise, not the car actually stopping), so a multi-block dispatch (e.g. a 2-hour BOOST) does not need a fresh confirming read in every single block to stay trusted throughout. As a safety net against ever missing that "not charging" reading, a streak can only roll forward a bounded number of blocks (currently 4, i.e. 2 hours) without a fresh confirming read of its own before it lapses and needs a new one to resume - this stops a permanently-missed negative edge from quietly trusting every future block indefinitely.
      **This level requires a genuine car_charging_now sensor to be configured for the car** (a real charger/car-reported signal, not a load-signature guess) - without one there is no reliable way to tell a real EV charging session apart from something else drawing similar power (e.g. a kettle), so `started` falls back to behaving as `none` for that car, with a startup warning, until **car_charging_now** is set for it. In a multi-car install this is decided per car, so one car can use `started` while another without the sensor falls back independently.
...
This setting and **switch.predbat_octopus_intelligent_limit_future_slots** above answer two different questions and can be used together or independently: this one asks whether Octopus is likely to *honour* a slot, that one asks whether your car still *needs* it. A future daytime slot has to clear both to be treated as cheap for the house battery, so enabling both is stricter than either alone. Both leave the fixed 23:30-05:30 window alone.
