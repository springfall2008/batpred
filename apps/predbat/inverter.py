...
class Inverter:
            ...
            for command in restart_command:
                ...
                if service:
                    ...
                    if self.base.get_arg("set_system_notify"):
                        self.base.call_notify("{}: Auto-restart service {} called due to: {}".format(self.base.prefix, service, reason))
                    self.sleep(15)
    ...
    def adjust_reserve(self, reserve):
        ...
        if current_reserve != reserve:
            ...
            if self.base.set_inverter_notify:
                self.base.call_notify("{}: Inverter {} Target Reserve has been changed to {}% at {}".format(self.base.prefix, self.id, dp0(reserve), self.base.time_now_str()))
        ...
        *If the inverter uses current rather than power we create a dummy entity for the power anyway but also write to the current entity
        ...
        if abs(current_rate - new_rate) > (self.battery_rate_max_charge * MINUTE_WATT / 20):
            ...
            if notify and self.base.set_inverter_notify:
                self.base.call_notify("{}: Inverter {} charge rate changes to {}W at {}".format(self.base.prefix, self.id, new_rate, self.base.time_now_str()))
    ...
    def adjust_discharge_rate(self, new_rate, notify=True):
        ...
        *If the inverter uses current rather than power we create a dummy entity for the power anyway but also write to the current entity
        ...
        if abs(current_rate - new_rate) > (self.battery_rate_max_discharge * MINUTE_WATT / 20):
            ...
            if notify and self.base.set_inverter_notify:
                self.base.call_notify("{}: Inverter {} discharge rate changes to {}W at {}".format(self.base.prefix, self.id, new_rate, self.base.time_now_str()))
    ...
    def adjust_battery_target(self, soc, isCharging=False, isExporting=False):
        ...
        if current_soc != soc:
            ...
            if self.base.set_inverter_notify:
                self.base.call_notify("{}: Inverter {} Target SoC has been changed to {}% at {}".format(self.base.prefix, self.id, soc, self.base.time_now_str()))
    ...
    def adjust_pause_mode(self, pause_charge=False, pause_discharge=False):
        ...
        if new_pause_mode != old_pause_mode:
            ...
            if self.base.set_inverter_notify:
                self.base.call_notify("{}: Inverter {} pause mode to set {} at time {}".format(self.base.prefix, self.id, new_pause_mode, self.base.time_now_str()))
    ...
    def adjust_inverter_mode(self, force_export, changed_start_end=False):
        ...
        if old_inverter_mode != new_inverter_mode:
            ...
            # Notify
            if self.base.set_inverter_notify:
                self.base.call_notify("{}: Inverter {} Force export set to {} at time {}".format(self.base.prefix, self.id, force_export, self.base.time_now_str()))
    ...
    def adjust_force_export(self, force_export, new_start_time=None, new_end_time=None):
        ...
        if changed_start_end:
            if self.base.set_inverter_notify:
                self.base.call_notify("{}: Inverter {} Export time slot set to {} - {} at time {}".format(self.base.prefix, self.id, new_start, new_end, self.base.time_now_str()))
    ...
    def disable_charge_window(self, notify=True):
        ...
        if old_charge_schedule_enable == "on" or old_charge_schedule_enable == "enable":
            ...
            if self.base.set_inverter_notify and notify:
                self.base.call_notify("{}: Inverter {} Disabled scheduled charging at {}".format(self.base.prefix, self.id, self.base.time_now_str()))
    ...
    def adjust_charge_window(self, charge_start_time, charge_end_time, minutes_now):
        ...
        if new_start != old_start or new_end != old_end or (self.inv_charge_time_format in ["H M", "H:M-H:M"]):
            ...
            if self.base.set_inverter_notify:
                self.base.call_notify("{}: Inverter {} Charge window change to: {} - {} at {}".format(self.base.prefix, self.id, new_start, new_end, self.base.time_now_str()))
        ...
        if (old_charge_schedule_enable == "off" or have_disabled) and (new_start != new_end):
            ...
            if old_charge_schedule_enable == "off" and self.base.set_inverter_notify:
                self.base.call_notify("{}: Inverter {} Enabling scheduled charging at {}".format(self.base.prefix, self.id, self.base.time_now_str()))
