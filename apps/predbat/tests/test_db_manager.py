...
def test_db_manager_error_handling(my_predbat=None):
    ...
    async def run_test():
        ...
        try:
            ...
            if history:
                assert len(history) == 0 or (len(history) == 1 and len(history[0]) == 0), \
                    f"Expected empty history for non-existent entity, got {history}"
...
def test_db_manager_persistence(my_predbat=None):
    ...
    async def run_test():
        ...
        try:
            ...
            assert len(all_entities_after) == len(all_entities), \
                f"Entity count mismatch: {len(all_entities_after)} after restart vs {len(all_entities)} before"
            ...
            for entity_id, expected_state, expected_attributes in test_data:
                ...
                assert retrieved["state"] == expected_state, \
                    f"State mismatch for {entity_id}: expected {expected_state}, got {retrieved['state']}"
                
                # Verify all attributes are preserved
                for attr_key, attr_value in expected_attributes.items():
                    assert attr_key in retrieved["attributes"], \
                        f"Attribute {attr_key} missing for {entity_id} after restart"
                    assert retrieved["attributes"][attr_key] == attr_value, \
                        f"Attribute {attr_key} mismatch for {entity_id}: expected {attr_value}, got {retrieved['attributes'][attr_key]}"
