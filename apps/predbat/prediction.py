...
class Prediction(PredictionBatch):
    ...
    def run_prediction(self, charge_limit, charge_window, export_window, export_limits, pv_scenario, end_record, save=None, step=PREDICT_STEP, cache=False):
        ...
        while minute < self.forecast_minutes:
            ...
            elif set_export_freeze and export_window_active and not is_anti_clipping and export_limit_now < 100.0 and (export_limit_now == 99.0 or set_export_freeze_only):
            ...
            if inverter_hybrid:
                ...
                if total_inverted > inverter_limit:
                    ...
                    if (getattr(self, "clipping_buffer_enable", False) or self.clipping_buffer_kwh > 0) and clipping_cost_weight > 0:
                        metric += (pv_ac_before - pv_ac_no_loss) * max(export_rate, 0.0) * clipping_cost_weight * 5
                    total_inverted = get_total_inverted(battery_draw, pv_dc, pv_ac, inverter_loss, inverter_hybrid)
            else:
                total_inverted = get_total_inverted(battery_draw, pv_dc, pv_ac, inverter_loss, inverter_hybrid)
                if total_inverted > inverter_limit:
            ...
            if diff < 0 and abs(diff) > export_limit:
                ...
                if (getattr(self, "clipping_buffer_enable", False) or self.clipping_buffer_kwh > 0) and clipping_cost_weight > 0:
                    metric += (pv_ac_before - pv_ac) * max(export_rate, 0.0) * clipping_cost_weight * 5
