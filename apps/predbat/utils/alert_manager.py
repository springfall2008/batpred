...
class AlertManager:
    """
    Object-oriented alert processing manager
    Handles weather alert feeds and geographical filtering
    """
    ...
    def process_alerts(self, testing=False):
        ...
        alerts = self.base.get_arg("alerts", {})
        if not alerts:
            return
    ...
    def apply_alerts(self, alerts, keep):
        ...
        if alerts:
            for alert in alerts:
                ...
                if onset and expires:
                    onset_minutes = int((onset - self.base.midnight_utc).total_seconds() / 60)
                    expires_minutes = int((expires - self.base.midnight_utc).total_seconds() / 60)
                    if expires_minutes >= self.base.minutes_now:
                        ...
                        for minute in range(onset_minutes, expires_minutes):
                            ...
                            if minute == self.base.minutes_now:
                                active_alert_text = alert.get("event") + " until " + str(expires)
                                active_alert = True

        alert_keep = alert_active_keep.get(self.base.minutes_now, 0)
        alert_show = []
        for alert in alerts:
            item = {}
        ...
        self.base.dashboard_item(self.base.prefix + ".alerts", state=active_alert_text, attributes={"friendly_name": "Weather alerts", "icon": "mdi:alert-outline", "keep": alert_keep, "alerts": alert_show})
