...
class Fetch:
    ...
    def get_car_charging_planned(self):
        ...
        for car_n in range(self.num_cars):
            ...
            # "completed" for that car. It needs replans to have all but stopped for a whole slot, so
    ...
    def fetch_config_options(self):
        ...
        if self.trust_future_dynamic_iog_slots == "started":
            # "started" is the one trust level that leans on a live car_charging_now reading rather
            # than only Octopus's own dispatch records (see rate_add_io_slots()) - so it needs a
            # genuine car-reported sensor behind it. Without one it falls back to behaving as
            # "completed" for that car (checked again, from scratch, inside rate_add_io_slots()
            # itself - this block is only the one-time startup warning, not the actual gate) rather
            # than guessing "is the car charging" from some other live signal (e.g. inferring it
            # from a load-power jump), which is exactly the false-positive risk (a kettle mistaken
            # for the car) that blocked this feature in review (#4885) - car_charging_now is
            # deliberately a distinct, charger/car-reported sensor (docs/car-charging.md), not a
            # load-signature guess, so requiring it closes that gap rather than papering over it.
                ...
                if not self.has_car_charging_now_sensor(car_n):
                    if car_n not in self.trust_iog_no_sensor_warned:
                        self.log(
                            ...
                            "'started' needs a real charger/car-reported sensor to trust the current dispatch slot early, so this car will behave as 'completed' "
                            "until car_charging_now is set for it.".format(car_n)
                        )
                        self.record_status(
                            "Warn: trust_future_dynamic_iog_slots is 'started' for car {} but car_charging_now is not configured - behaves as 'completed'".format(car_n),
                            had_errors=True,
                        )
