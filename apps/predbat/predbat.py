...
class Inverter:
    async def async_self_test(self, minutes_now):
        self.log(f"======= INVERTER CONTROL SELF TEST START - REST={self.rest_api} ========")
        await self.async_adjust_battery_target(99, False)
        await self.async_adjust_battery_target(100, False)
        await self.async_adjust_charge_rate(215)
        await self.async_adjust_charge_rate(self.battery_rate_max_charge)
        await self.async_adjust_discharge_rate(220)
        await self.async_adjust_discharge_rate(self.battery_rate_max_discharge)
        await self.async_adjust_reserve(100)
        await self.async_adjust_reserve(6)
        await self.async_adjust_reserve(4)
        await self.async_disable_charge_window()
        ...
        await self.async_adjust_charge_window(timeb, timed, minutes_now)
        await self.async_adjust_charge_window(timea, timec, minutes_now)
        await self.async_adjust_force_discharge(False, timec, timed)
        await self.async_adjust_force_discharge(True, timea, timeb)
        await self.async_adjust_force_discharge(False)
        self.log("======= INVERTER CONTROL SELF TEST END ========")

        if self.rest_api:
            self.rest_api = None
            self.rest_data = None
            await self.async_self_test(minutes_now)
        exit

    async def async_auto_restart(self, reason):
        """
        Attempt to restart the services required
        """
        if self.base.restart_active:
            self.log("WARN: Inverter control auto restart already active, waiting...")
            return

        # Trigger restart
        self.log("WARN: Inverter control auto restart trigger: {}".format(reason))
        restart_command = await self.async_get_arg("auto_restart", [])
        if restart_command:
            ...
            for command in restart_command:
                ...
                if addon:
                    addon = await self.base.async_resolve_arg(service, addon, indirect=False)
                entity_id = command.get("entity_id", None)
                if entity_id:
                    entity_id = await self.base.async_resolve_arg(service, entity_id, indirect=False)
                if shell:
                ...
                if service:
                    if addon:
                        ...
                        await self.base.call_service(service, addon=addon)
                    elif entity_id:
                        self.log("Calling restart service {} with entity_id {}".format(service, entity_id))
                        await self.base.call_service(service, entity_id=entity_id)
                    else:
                        self.log("Calling restart service {}".format(service))
                        await self.base.call_service(service)
                    await self.base.call_notify("Auto-restart service {} called due to: {}".format(service, reason))
    ...
    async def async_init_inverter(self):
        ...
        self.reserve_percent = await self.async_get_arg("battery_min_soc", default=4.0, index=self.id)
        self.reserve_percent_current = await self.async_get_arg("battery_min_soc", default=4.0, index=self.id)
        self.battery_scaling = await self.async_get_arg("battery_scaling", default=1.0, index=self.id)
        ...
        self.inverter_type = await self.async_get_arg("inverter_type", "GE", indirect=False, index=self.id)
        ...
        self.reserve_max = await self.async_get_arg("inverter_reserve_max", 100)
        ...
        self.inv_mqtt_topic = await self.async_get_arg("mqtt_topic", "Sofar2mqtt")
        ...
        if self.inverter_type == "GE":
            self.rest_api = await self.async_get_arg("givtcp_rest", None, indirect=False, index=self.id)
            if self.rest_api:
                if not quiet:
                    self.log("Inverter {} using Rest API {}".format(self.id, self.rest_api))
                self.rest_data = await self.async_rest_readData()
                if not self.rest_data:
                    await self.async_auto_restart("REST read failure")

        # Timed pause support?
        if self.inv_has_timed_pause:
            entity_mode = await self.async_get_arg("pause_mode", indirect=False, index=self.id)
            if entity_mode:
                old_pause_mode = await self.base.get_state(entity_mode)
        ...
        if self.rest_data and ("Invertor_Details" in self.rest_data):
            ...
            if "Invertor_Max_Bat_Rate" in idetails:
                ...
                self.battery_rate_max_raw = await self.async_get_arg("charge_rate", attribute="max", index=self.id, default=2600.0)
            ...
            self.soc_max = await self.async_get_arg("soc_max", default=10.0, index=self.id) * self.battery_scaling
            self.nominal_capacity = self.soc_max

            self.battery_voltage = 52.0
            if "battery_voltage" in self.base.args:
                await self.async_get_arg("battery_voltage", index=self.id, default=52.0)

            if self.inverter_type in ["GE", "GEC", "GEE"]:
                self.battery_rate_max_raw = await self.async_get_arg("charge_rate", attribute="max", index=self.id, default=2600.0)
            elif "battery_rate_max" in self.base.args:
                self.battery_rate_max_raw = await self.async_get_arg("battery_rate_max", index=self.id, default=2600.0)
            else:
                self.battery_rate_max_raw = 2600.0

            ivtime = await self.async_get_arg("inverter_time", index=self.id, default=None)

        # Battery can not be zero size
        if self.soc_max <= 0:
            self.log("ERROR: Reported battery size from REST is {}, but it must be >0".format(self.soc_max))
            raise ValueError
        ...
        if ivtime:
            try:
                ...
                try:
                    ...
                    try:
                        tz = pytz.timezone(await self.async_get_arg("timezone", "Europe/London"))
                        self.inverter_time = tz.localize(datetime.strptime(ivtime, self.inv_clock_time_format))
                    except (ValueError, TypeError):
                        self.log(f"Warn: Unable to read inverter time string {ivtime} using formats {[TIME_FORMAT, TIME_FORMAT_OCTOPUS, self.inv_clock_time_format]}")
                        self.inverter_time = None
                        await self.async_auto_restart("Unable to read inverter time")
        ...
        if self.inverter_time:
            ...
            local_tz = pytz.timezone(await self.async_get_arg("timezone", "Europe/London"))
            now_utc = datetime.now(local_tz)

            tdiff = self.inverter_time - now_utc
            tdiff = self.base.dp2(tdiff.seconds / 60 + tdiff.days * 60 * 24)
            if not quiet:
                self.log("Invertor time {} AppDaemon time {} difference {} minutes".format(self.inverter_time, now_utc, tdiff))
            if abs(tdiff) >= 5:
                self.log(
                ...
                # Trigger restart
                await self.async_auto_restart("Clock skew >=5 minutes")
            else:
        ...
        battery_min_soc = await self.async_get_arg("battery_min_soc", default=4.0, index=self.id)
        self.reserve_min = await self.async_get_arg("set_reserve_min")
        if self.reserve_min < battery_min_soc:
            self.log(f"Increasing set_reserve_min from {self.reserve_min}%  to battery_min_soc of {battery_min_soc}%")
            await self.base.async_expose_config("set_reserve_min", battery_min_soc)
            self.reserve_min = battery_min_soc

        self.log(f"Reserve min: {self.reserve_min}% Battery_min:{battery_min_soc}%")
        ...
        if "inverter_limit" in self.base.args:
            self.inverter_limit = await self.async_get_arg("inverter_limit", self.inverter_limit, index=self.id) / MINUTE_WATT
        if "export_limit" in self.base.args:
            self.export_limit = await self.async_get_arg("export_limit", self.inverter_limit, index=self.id) / MINUTE_WATT
        ...
        if not self.inv_has_charge_enable_time:
            ...
            self.base.args["scheduled_charge_enable"][self.id] = await self.async_create_entity("scheduled_charge_enable", "on")
        ...
        if not self.inv_has_discharge_enable_time:
            ...
            self.base.args["scheduled_discharge_enable"][self.id] = await self.async_create_entity("scheduled_discharge_enable", "on")

        if not self.inv_has_reserve_soc:
            self.base.args["reserve"] = await self.async_create_entity("reserve", self.reserve, device_class="battery", uom="%")

        if not self.inv_has_target_soc:
            self.base.args["charge_limit"] = await self.async_create_entity("charge_limit", 100, device_class="battery", uom="%")

        if self.inv_output_charge_control != "power":
            self.base.args["charge_rate"] = await self.async_create_entity("charge_rate", int(self.battery_rate_max_charge * MINUTE_WATT), uom="W", device_class="power")
            self.base.args["discharge_rate"] = await self.async_create_entity("discharge_rate", int(self.battery_rate_max_discharge * MINUTE_WATT), uom="W", device_class="power")

        if not self.inv_has_ge_inverter_mode:
            if "inverter_mode" not in self.base.args:
                self.base.args["inverter_mode"] = ["Eco", "Eco", "Eco", "Eco"]
            self.base.args["inverter_mode"][self.id] = await self.async_create_entity("inverter_mode", "Eco")

        if self.inv_charge_time_format != "HH:MM:SS":
            for x in ["charge", "discharge"]:
                for y in ["start", "end"]:
                    self.base.args[f"{x}_{y}_time"] = await self.async_create_entity(f"{x}_{y}_time", "23:59:00")

    async def async_find_charge_curve(self, discharge):
        """
        ...
        soc_kwh_sensor = await self.async_get_arg("soc_kw", indirect=False, index=self.id)
        if discharge:
            charge_rate_sensor = await self.async_get_arg("discharge_rate", indirect=False, index=self.id)
        else:
            charge_rate_sensor = await self.async_get_arg("charge_rate", indirect=False, index=self.id)
        predbat_status_sensor = "predbat.status"
        battery_power_sensor = await self.async_get_arg("battery_power", indirect=False, index=self.id)
        final_curve = {}
        final_curve_count = {}
        ...
        if soc_kwh_sensor and charge_rate_sensor and battery_power_sensor and predbat_status_sensor:
            ...
            soc_kwh_data = await self.base.async_get_history_async(entity_id=soc_kwh_sensor, days=self.base.max_days_previous)
            charge_rate_data = await self.base.async_get_history_async(entity_id=charge_rate_sensor, days=self.base.max_days_previous)
            predbat_status_data = await self.base.async_get_history_async(entity_id=predbat_status_sensor, days=self.base.max_days_previous)
            battery_power_data = await self.base.async_get_history_async(entity_id=battery_power_sensor, days=self.base.max_days_previous)

            if soc_kwh_data and charge_rate_data and charge_rate_data and battery_power_data:
                soc_kwh = await self.base.async_minute_data(
                    soc_kwh_data[0],
                ...
                charge_rate = await self.base.async_minute_data(
                    charge_rate_data[0],
                    self.base.max_days_previous,
                    self.base.now_utc,
                    "state",
                ...
                predbat_status = await self.base.async_minute_data_state(predbat_status_data[0], self.base.max_days_previous, self.base.now_utc, "state", "last_updated")
                battery_power = await self.base.async_minute_data(
    ...
    async def async_create_entity(self, entity_name, value, uom=None, device_class="None"):
        """
        Create dummy entities required by non GE inverters to mimic GE behaviour
        """
        if "prefix" in self.base.args:
            prefix = await self.async_get_arg("prefix", indirect=False)
        else:
            prefix = "prefix"
        ...
        if not self.base.entity_exists(entity_id):
            ...
            await entity.set_state(state=value, attributes=attributes)
        return entity_id

    async def async_update_status(self, minutes_now, quiet=False):
        """
        ...
        if self.rest_data:
            ...
            self.log(
                "Inverter {} scheduled_charge_enable {} scheduled_discharge_enable {}".format(
                    self.id, await self.async_get_arg("scheduled_charge_enable", "on", index=self.id), await self.async_get_arg("scheduled_discharge_enable", "off", index=self.id)
                )
            )
            self.charge_enable_time = await self.async_get_arg("scheduled_charge_enable", "on", index=self.id) == "on"
            self.discharge_enable_time = await self.async_get_arg("scheduled_discharge_enable", "off", index=self.id) == "on"
            self.charge_rate_now = await self.async_get_arg("charge_rate", index=self.id, default=2600.0) / MINUTE_WATT
            self.discharge_rate_now = await self.async_get_arg("discharge_rate", index=self.id, default=2600.0) / MINUTE_WATT
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                if "soc_percent" in self.base.args:
                    self.soc_kw = await self.async_get_arg("soc_percent", default=0.0, index=self.id) * self.soc_max * self.battery_scaling / 100.0
                else:
                    self.soc_kw = await self.async_get_arg("soc_kw", default=0.0, index=self.id) * self.battery_scaling
        ...
        if self.rest_data and ("Power" in self.rest_data):
            ...
            self.battery_power = await self.async_get_arg("battery_power", default=0.0, index=self.id)
            self.pv_power = await self.async_get_arg("pv_power", default=0.0, index=self.id)
            self.load_power = await self.async_get_arg("load_power", default=0.0, index=self.id)

            for i in range(1, self.inv_num_load_entities):
                self.load_power += await self.async_get_arg(f"load_power_{i}", default=0.0, index=self.id)

        self.battery_voltage = await self.async_get_arg("battery_voltage", default=52.0, index=self.id)
        ...
        if self.charge_enable_time or not self.inv_has_charge_enable_time:
            ...
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    charge_start_time = datetime.strptime(await self.async_get_arg("charge_start_time", index=self.id), "%H:%M:%S")
                    charge_end_time = datetime.strptime(await self.async_get_arg("charge_end_time", index=self.id), "%H:%M:%S")
                else:
            ...
            if not self.inv_has_charge_enable_time:
                entity = self.base.get_entity(await self.async_get_arg("scheduled_charge_enable", indirect=False, index=self.id))
                if charge_start_time == charge_end_time:
                    self.charge_enable_time = False
                else:
                    self.charge_enable_time = True
                await self.async_write_and_poll_switch("scheduled_charge_enable", entity, self.charge_enable_time)
        ...
        if not quiet:
            self.log("Inverter {} scheduled charge enable is {}".format(self.id, self.charge_enable_time))
        ...
        if not quiet:
            self.log("Inverter {} charge windows currently {}".format(self.id, self.charge_window))
        ...
        if self.charge_enable_time:
            if self.rest_data:
                ...
                self.current_charge_limit = float(await self.async_get_arg("charge_limit", index=self.id, default=100.0))
        else:
            self.current_charge_limit = 0.0
        ...
        if not quiet:
            if self.charge_enable_time:
                ...
                self.log("Inverter {} Charge settings: timed charged is disabled, power {} kW".format(self.id, round(self.charge_rate_now * 60.0, 2)))
        ...
        if self.rest_data:
            ...
            discharge_start = datetime.strptime(await self.async_get_arg("discharge_start_time", index=self.id), "%H:%M:%S")
        ...
        if not self.inv_has_discharge_enable_time:
            ...
            entity = self.base.get_entity(await self.async_get_arg("scheduled_discharge_enable", indirect=False, index=self.id))
            await self.async_write_and_poll_switch("scheduled_discharge_enable", entity, self.discharge_enable_time)
            self.log("Inverter {} {} set to {}".format(self.id, await self.async_get_arg("scheduled_discharge_enable", indirect=False, index=self.id), self.discharge_enable_time))
        ...
        if not quiet:
            self.log("Inverter {} scheduled discharge enable is {}".format(self.id, self.discharge_enable_time))
        ...
        if not quiet:
            self.log("Inverter {} discharge windows currently {}".format(self.id, self.discharge_window))

        if INVERTER_TEST:
            await self.async_self_test(minutes_now)

    async def async_mimic_target_soc(self, current_charge_limit):
        """
        ...
        Returns:
            None
        """
        charge_power = await self.async_get_arg("charge_rate", index=self.id, default=2600.0)
        if current_charge_limit == 0:
            await self.async_alt_charge_discharge_enable("eco", True)  # ECO Mode
        elif self.soc_percent > float(current_charge_limit):
            # If current SOC is above Target SOC, turn Grid Charging off
            await self.async_alt_charge_discharge_enable("charge", False)
            self.log(f"Current SOC {self.soc_percent}% is greater than Target SOC {current_charge_limit}. Grid Charge disabled.")
        elif self.soc_percent == float(current_charge_limit):  # If SOC target is reached
            await self.async_alt_charge_discharge_enable("charge", True)  # Make sure charging is on
            if self.inv_output_charge_control == "current":
                await self.async_set_current_from_power("charge", (0))  # Set charge current to zero (i.e hold SOC)
                self.log(f"Current SOC {self.soc_percent}% is same as Target SOC {current_charge_limit}. Grid Charge enabled, Amps rate set to 0.")
        else:
            # If we drop below the target, turn grid charging back on and make sure the charge current is correct
            await self.async_alt_charge_discharge_enable("charge", True)
            if self.inv_output_charge_control == "current":
                await self.async_set_current_from_power("charge", charge_power)  # Write previous current setting to inverter
                self.log(f"Current SOC {self.soc_percent}% is less than Target SOC {current_charge_limit}. Grid Charge enabled, amp rate written to inverter.")
            self.log(
                f"Current SOC {self.soc_percent}% is less than Target SOC {current_charge_limit}. Grid charging enabled with charge current set to {await self.async_get_arg('timed_charge_current', index=self.id, default=65):0.2f}"
            )

    async def async_adjust_reserve(self, reserve):
        """
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                current_reserve = await self.async_get_arg("reserve", index=self.id, default=0.0)
        ...
        if current_reserve != reserve:
            self.log("Inverter {} Current Reserve is {} % and new target is {} %".format(self.id, current_reserve, reserve))
            if SIMULATE:
                self.base.sim_reserve = reserve
            else:
                if self.rest_data:
                    await self.async_rest_setReserve(reserve)
                else:
                    entity_soc = self.base.get_entity(await self.async_get_arg("reserve", indirect=False, index=self.id))
                    await self.async_write_and_poll_value("reserve", entity_soc, reserve)
                if self.base.set_inverter_notify:
                    await self.base.call_notify("Predbat: Inverter {} Target Reserve has been changed to {} at {}".format(self.id, reserve, self.base.time_now_str()))
            await self.async_mqtt_message(topic="set/reserve", payload=reserve)
        else:
            self.log("Inverter {} Current reserve is {} already at target".format(self.id, current_reserve))

    async def async_adjust_charge_rate(self, new_rate, notify=True):
        """
        ...
        *If the inverter uses current rather than power we create a dummy entity for the power anyway but also write to the current entity
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                current_rate = await self.async_get_arg("charge_rate", index=self.id, default=2600.0)

        if abs(current_rate - new_rate) > 100:
            self.log("Inverter {} current charge rate is {} and new target is {}".format(self.id, current_rate, new_rate))
            if SIMULATE:
                self.base.sim_charge_rate_now = new_rate
            else:
                if self.rest_data:
                    await self.async_rest_setChargeRate(new_rate)
                else:
                    if "charge_rate" in self.base.args:
                        entity = self.base.get_entity(await self.async_get_arg("charge_rate", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("charge_rate", entity, new_rate, fuzzy=(self.battery_rate_max_charge * MINUTE_WATT / 12))

                    if self.inv_output_charge_control == "current":
                        await self.async_set_current_from_power("charge", new_rate)

                if notify and self.base.set_inverter_notify:
                    await self.base.call_notify("Predbat: Inverter {} charge rate changes to {} at {}".format(self.id, new_rate, self.base.time_now_str()))
            await self.async_mqtt_message(topic="set/charge_rate", payload=new_rate)

    async def async_adjust_discharge_rate(self, new_rate, notify=True):
        """
        ...
        *If the inverter uses current rather than power we create a dummy entity for the power anyway but also write to the current entity
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                current_rate = await self.async_get_arg("discharge_rate", index=self.id, default=2600.0)

        if abs(current_rate - new_rate) > 100:
            self.log("Inverter {} current discharge rate is {} and new target is {}".format(self.id, current_rate, new_rate))
            if SIMULATE:
                self.base.sim_discharge_rate_now = new_rate
            else:
                if self.rest_data:
                    await self.async_rest_setDischargeRate(new_rate)
                else:
                    if "discharge_rate" in self.base.args:
                        entity = self.base.get_entity(await self.async_get_arg("discharge_rate", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("discharge_rate", entity, new_rate, fuzzy=(self.battery_rate_max_discharge * MINUTE_WATT / 25))

                    if self.inv_output_charge_control == "current":
                        await self.async_set_current_from_power("discharge", new_rate)

                if notify and self.base.set_inverter_notify:
                    await self.base.call_notify("Predbat: Inverter {} discharge rate changes to {} at {}".format(self.id, new_rate, self.base.time_now_str()))
            await self.async_mqtt_message(topic="set/discharge_rate", payload=new_rate)

    async def async_adjust_battery_target(self, soc, isCharging=False):
        """
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                current_soc = int(float(await self.async_get_arg("charge_limit", index=self.id, default=100.0)))

        if current_soc != soc:
            self.log("Inverter {} Current charge limit is {} % and new target is {} %".format(self.id, current_soc, soc))
            self.current_charge_limit = soc
            if SIMULATE:
                self.base.sim_soc = soc
            else:
                if self.rest_data:
                    await self.async_rest_setChargeTarget(soc)
                else:
                    entity_soc = self.base.get_entity(await self.async_get_arg("charge_limit", indirect=False, index=self.id))
                    await self.async_write_and_poll_value("charge_limit", entity_soc, soc)

                if self.base.set_inverter_notify:
                    await self.base.call_notify("Predbat: Inverter {} Target SOC has been changed to {} % at {}".format(self.id, soc, self.base.time_now_str()))
            await self.async_mqtt_message(topic="set/target_soc", payload=soc)
        else:
            self.log("Inverter {} Current Target SOC is {} already at target".format(self.id, current_soc))

        # Inverters that need on/off controls rather than target SOC
        if not self.inv_has_target_soc:
            if isCharging:
                await self.async_mimic_target_soc(soc)
            else:
                await self.async_mimic_target_soc(0)

    async def async_write_and_poll_switch(self, name, entity, new_value):
        """
        ...
        current_state = await entity.get_state()
        ...
        while current_state != new_value and retry < 6:
            ...
            if domain == "sensor":
                if new_value:
                    await entity.set_state(state="on")
                else:
                    await entity.set_state(state="off")
            else:
                if new_value:
                    await entity.call_service("turn_on")
                else:
                    await entity.call_service("turn_off")

            await asyncio.sleep(self.inv_write_and_poll_sleep)
            current_state = await entity.get_state()
            if isinstance(current_state, str):
                current_state = current_state.lower() in ["on", "enable", "true"]

        if current_state == new_value:
            self.log("Inverter {} Wrote {} to {} successfully and got {}".format(self.id, name, new_value, await entity.get_state()))
            return True
        else:
            self.log("WARN: Inverter {} Trying to write {} to {} didn't complete got {}".format(self.id, name, new_value, await entity.get_state()))
            await self.base.async_record_status("Warn - Inverter {} write to {} failed".format(self.id, name), had_errors=True)
            return False

    async def async_write_and_poll_value(self, name, entity, new_value, fuzzy=0):
        ...
        while (not matched) and (retry < 6):
            retry += 1
            if domain == "sensor":
                await entity.set_state(state=new_value)
            else:
                # if isinstance(new_value, str):
                await entity.call_service("set_value", value=new_value)
        ...
        if retry == 0:
            self.log(f"Inverter {self.id} No write needed for {name}: {new_value} == {current_state}")
            return True
        elif matched:
            self.log(f"Inverter {self.id} Wrote {new_value} to {name}, successfully now {current_state}")
            return True
        else:
            self.log(f"WARN: Inverter {self.id} Trying to write {new_value} to {name} didn't complete got {current_state}")
            await self.base.async_record_status(f"Warn - Inverter {self.id} write to {name} failed", had_errors=True)
            return False

    async def async_write_and_poll_option(self, name, entity, new_value):
        """
        GivTCP Workaround, keep writing until correct
        """
        for retry in range(6):
            await entity.call_service("select_option", option=new_value)
            await asyncio.sleep(self.inv_write_and_poll_sleep)
            old_value = await entity.get_state()
            if old_value == new_value:
                self.log("Inverter {} Wrote {} to {} successfully".format(self.id, name, new_value))
                return True
        self.log("WARN: Inverter {} Trying to write {} to {} didn't complete got {}".format(self.id, name, new_value, await entity.get_state()))
        await self.base.async_record_status("Warn - Inverter {} write to {} failed".format(self.id, name), had_errors=True)
        return False

    async def async_adjust_pause_mode(self, pause_charge=False, pause_discharge=False):
        """
        ...
        entity_mode = await self.async_get_arg("pause_mode", indirect=False, index=self.id)
        entity_start = await self.async_get_arg("pause_start_time", indirect=False, index=self.id)
        entity_end = await self.async_get_arg("pause_end_time", indirect=False, index=self.id)
        old_pause_mode = None
        old_start_time = None
        old_end_time = None
        ...
        if entity_start:
            old_start_time = await self.base.get_state(entity_start)
        ...
        if entity_end:
            old_end_time = await self.base.get_state(entity_end)
        ...
        if old_start_time and old_start_time != new_start_time:
            ...
            await entity_start.set_state(state=new_start_time)
            self.log("Inverter {} set pause start time to {}".format(self.id, new_start_time))
        if old_end_time and old_end_time != new_end_time:
            # Don't poll as inverters with no registers will fail
            await entity_end.set_state(state=new_end_time)
            self.log("Inverter {} set pause end time to {}".format(self.id, new_end_time))

        # Set the mode
        if new_pause_mode != old_pause_mode:
            await self.async_write_and_poll_option("inverter_mode", entity_mode, new_pause_mode)

            if self.base.set_inverter_notify:
                await self.base.call_notify("Predbat: Inverter {} pause mode to set {} at time {}".format(self.id, new_pause_mode, self.base.time_now_str()))

            self.log("Inverter {} set pause mode to {}".format(self.id, new_pause_mode))

    async def async_adjust_inverter_mode(self, force_discharge, changed_start_end=False):
        """
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                if changed_start_end and not self.rest_data:
                    ...
                    self.log("Sleeping (workaround) as start/end of discharge window was just adjusted")
                    await asyncio.sleep(30)
                old_inverter_mode = await self.async_get_arg("inverter_mode", index=self.id)
        ...
        if old_inverter_mode != new_inverter_mode:
            if SIMULATE:
                ...
                if self.rest_data:
                    await self.async_rest_setBatteryMode(new_inverter_mode)
                else:
                    entity = self.base.get_entity(await self.async_get_arg("inverter_mode", indirect=False, index=self.id))
                    if self.inv_has_ge_inverter_mode:
                        await self.async_write_and_poll_option("inverter_mode", entity, new_inverter_mode)
                    else:
                        await self.async_write_and_poll_value("inverter_mode", entity, new_inverter_mode)

                # Notify
                if self.base.set_inverter_notify:
                    await self.base.call_notify("Predbat: Inverter {} Force discharge set to {} at time {}".format(self.id, force_discharge, self.base.time_now_str()))

            self.log("Inverter {} set force discharge to {}".format(self.id, force_discharge))

    async def async_adjust_idle_time(self, charge_start=None, charge_end=None, discharge_start=None, discharge_end=None):
        """
        ...
        charge_start_minutes, charge_end_minutes = await self.async_window2minutes(self.track_charge_start, self.track_charge_end, "%H:%M:%S", minutes_now)
        discharge_start_minutes, discharge_end_minutes = await self.async_window2minutes(self.track_discharge_start, self.track_discharge_end, "%H:%M:%S", minutes_now)
        ...
        self.log("Adjust idle time computed idle is {}-{}".format(idle_start, idle_end))

        # Write idle start/end time
        if self.inv_has_idle_time:
            entity_idle_start_time = self.base.get_entity(await self.async_get_arg("idle_start_time", indirect=False, index=self.id))
            entity_idle_end_time = self.base.get_entity(await self.async_get_arg("idle_end_time", indirect=False, index=self.id))

            if entity_idle_start_time and entity_idle_end_time:
                old_start = await self.async_get_arg("idle_start_time", index=self.id)
                old_end = await self.async_get_arg("idle_end_time", index=self.id)

                if old_start != idle_start:
                    self.log("Inverter {} set new idle start time to {} was {}".format(self.id, idle_start, old_start))
                    await self.async_write_and_poll_option("idle_start_time", entity_idle_start_time, idle_start)
                if old_end != idle_end:
                    self.log("Inverter {} set new idle end time to {} was {}".format(self.id, idle_end, old_end))
                    await self.async_write_and_poll_option("idle_end_time", entity_idle_end_time, idle_end)

    async def async_window2minutes(self, start, end, format, minutes_now):
        """
    ...
    async def async_adjust_force_discharge(self, force_discharge, new_start_time=None, new_end_time=None):
        """
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                old_start = await self.async_get_arg("discharge_start_time", index=self.id)
                old_end = await self.async_get_arg("discharge_end_time", index=self.id)
                old_discharge_enable = await self.async_get_arg("scheduled_discharge_enable", "off", index=self.id) == "on"
            else:
        ...
        if not force_discharge:
            await self.async_adjust_inverter_mode(force_discharge)

        self.log("Inverter {} Adjust force discharge to {}, change times from {} - {} to {} - {}".format(self.id, force_discharge, old_start, old_end, new_start, new_end))
        changed_start_end = False

        # Some inverters have an idle time setting
        if force_discharge:
            await self.async_adjust_idle_time(discharge_start=new_start, discharge_end=new_end)
        else:
            await self.async_adjust_idle_time(discharge_start="00:00:00", discharge_end="00:00:00")

        # Change start time
        if new_start and new_start != old_start:
            self.log("Inverter {} set new start time to {}".format(self.id, new_start))
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity_discharge_start_time = self.base.get_entity(await self.async_get_arg("discharge_start_time", indirect=False, index=self.id))
                    if self.inv_charge_time_entity_is_option:
                        await self.async_write_and_poll_option("discharge_start_time", entity_discharge_start_time, new_start)
                    else:
                        await self.async_write_and_poll_value("discharge_start_time", entity_discharge_start_time, new_start)

                    if self.inv_charge_time_format == "H M":
                        # If the inverter uses hours and minutes then write to these entities too
                        entity = self.base.get_entity(await self.async_get_arg("discharge_start_hour", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("discharge_start_hour", entity, int(new_start[:2]))
                        entity = self.base.get_entity(await self.async_get_arg("discharge_start_minute", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("discharge_start_minute", entity, int(new_start[3:5]))
                else:
        ...
        if new_end and new_end != old_end:
            self.log("Inverter {} Set new end time to {} was {}".format(self.id, new_end, old_end))
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    # Always write to this as it is the GE default
                    changed_start_end = True
                    entity_discharge_end_time = self.base.get_entity(await self.async_get_arg("discharge_end_time", indirect=False, index=self.id))
                    if self.inv_charge_time_entity_is_option:
                        await self.async_write_and_poll_option("discharge_end_time", entity_discharge_end_time, new_end)
                        # If the inverter uses hours and minutes then write to these entities too
                    else:
                        await self.async_write_and_poll_value("discharge_end_time", entity_discharge_end_time, new_end)

                    if self.inv_charge_time_format == "H M":
                        entity = self.base.get_entity(await self.async_get_arg("discharge_end_hour", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("discharge_end_hour", entity, int(new_end[:2]))
                        entity = self.base.get_entity(await self.async_get_arg("discharge_end_minute", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("discharge_end_minute", entity, int(new_end[3:5]))
                else:
        ...
        if ((new_end != old_end) or (new_start != old_start)) and self.inv_time_button_press:
            entity_id = await self.async_get_arg("charge_discharge_update_button", indirect=False, index=self.id)
            await self.async_press_and_poll_button(entity_id)

        # Change scheduled discharge enable
        if force_discharge and not old_discharge_enable:
            if not SIMULATE:
                entity = self.base.get_entity(await self.async_get_arg("scheduled_discharge_enable", indirect=False, index=self.id))
                await self.async_write_and_poll_switch("scheduled_discharge_enable", entity, True)
                self.log("Inverter {} Turning on scheduled discharge".format(self.id))
        elif not force_discharge and old_discharge_enable:
            if not SIMULATE:
                entity = self.base.get_entity(await self.async_get_arg("scheduled_discharge_enable", indirect=False, index=self.id))
                await self.async_write_and_poll_switch("scheduled_discharge_enable", entity, False)
        ...
        if self.rest_data and new_start and new_end and ((new_start != old_start) or (new_end != old_end)):
            changed_start_end = True
            if not SIMULATE:
                await self.async_rest_setDischargeSlot1(new_start, new_end)

        # Force discharge, turn it on after we change the window
        if force_discharge:
            await self.async_adjust_inverter_mode(force_discharge, changed_start_end=changed_start_end)

        # Notify
        if changed_start_end:
            if self.base.set_inverter_notify:
                await self.base.call_notify("Predbat: Inverter {} Discharge time slot set to {} - {} at time {}".format(self.id, new_start, new_end, self.base.time_now_str()))

    async def async_disable_charge_window(self, notify=True):
        """
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                old_charge_schedule_enable = await self.async_get_arg("scheduled_charge_enable", "on", index=self.id)

        await self.async_adjust_idle_time(charge_start="00:00:00", charge_end="00:00:00")

        if old_charge_schedule_enable == "on" or old_charge_schedule_enable == "enable":
            if not SIMULATE:
                # Enable scheduled charge if not turned on
                if self.rest_data:
                    await self.async_rest_enableChargeSchedule(False)
                else:
                    entity = self.base.get_entity(await self.async_get_arg("scheduled_charge_enable", indirect=False, index=self.id))
                    await self.async_write_and_poll_switch("scheduled_charge_enable", entity, False)
                    ...
                    if not self.inv_has_charge_enable_time and (self.inv_output_charge_control == "current"):
                        await self.async_enable_charge_discharge_with_time_current("charge", False)
                    else:
                        await self.async_adjust_charge_window(self.base.midnight_utc, self.base.midnight_utc, self.base.minutes_now)

                if self.base.set_inverter_notify and notify:
                    await self.base.call_notify("Predbat: Inverter {} Disabled scheduled charging at {}".format(self.id, self.base.time_now_str()))
            else:
                self.base.sim_charge_schedule_enable = "off"

            self.log("Inverter {} Turning off scheduled charge".format(self.id))
    ...
    async def async_alt_charge_discharge_enable(self, direction, enable):
        """
        ...
        if self.inverter_type == "GS":
            ...
            solax_modes = SOLAX_SOLIS_MODES_NEW if await self.async_get_arg("solax_modbus_new", True) else SOLAX_SOLIS_MODES

            entity_id = await self.async_get_arg("energy_control_switch", indirect=False, index=self.id)
            entity = self.base.get_entity(entity_id)
            switch = solax_modes.get(await entity.get_state(), 0)
            ...
            if new_switch != switch:
                self.log(f"Setting Solis Energy Control Switch to {new_switch} {new_mode} from {switch} {old_mode} for {direction} {enable}")
                await self.async_write_and_poll_option(name=entity_id, entity=entity, new_value=new_mode)
            else:
                self.log(f"Solis Energy Control Switch setting {switch} {new_mode} unchanged for {direction} {enable}")

        # MQTT
        if direction == "charge" and enable:
            await self.async_mqtt_message("set/charge", payload=int(self.battery_rate_max_charge * MINUTE_WATT))
        elif direction == "discharge" and enable:
            await self.async_mqtt_message("set/discharge", payload=int(self.battery_rate_max_discharge * MINUTE_WATT))
        else:
            await self.async_mqtt_message("set/auto", payload="true")

    async def async_mqtt_message(self, topic, payload):
        """
        Send an MQTT message via service
        """
        if self.inv_has_mqtt_api:
            await self.base.call_service("mqtt/publish", qos=1, retain=True, topic=(self.inv_mqtt_topic + "/" + topic), payload=payload)

    async def async_enable_charge_discharge_with_time_current(self, direction, enable):
        """
        Enable or disable timed charge/discharge
        """
        # To enable we set the current based on the required power
        if enable:
            power = await self.async_get_arg(f"{direction}_rate", index=self.id, default=2600.0)
            await self.async_set_current_from_power(direction, power)
        else:
            if self.inv_charge_time_format == "H M":
                ...
                for x in ["start", "end"]:
                    for y in ["hour", "minute"]:
                        ...
                        entity = self.base.get_entity(await self.async_get_arg(name, indirect=False, index=self.id))
                        await self.async_write_and_poll_value(name, entity, 0)
            else:
                await self.async_set_current_from_power(direction, 0)

    async def async_set_current_from_power(self, direction, power):
        """
        ...
        entity = self.base.get_entity(await self.async_get_arg(f"timed_{direction}_current", indirect=False, index=self.id))
        await self.async_write_and_poll_value(f"timed_{direction}_current", entity, new_current, fuzzy=1)

    async def async_call_service_template(self, service, data):
        """
        ...
        if service_template:
            if isinstance(service_template, str):
                ...
                for key in service_template:
                    if key == "service":
                        ...
                        value = await self.base.async_resolve_arg(service_template, value, indirect=False, index=self.id, default="", extra_args=data)
                        if value:
            ...
            if service_name:
                ...
                await self.base.call_service(service_name, **service_data)
            else:
    ...
    async def async_adjust_charge_immediate(self, target_soc):
        """
        ...
        if self.inv_has_service_api:
            if target_soc > 0:
                service_data = {
                    "device_id": await self.async_get_arg("device_id", index=self.id, default=""),
                    "target_soc": target_soc,
                    "power": int(self.battery_rate_max_charge * MINUTE_WATT),
                }
                await self.async_call_service_template("charge_start_service", service_data)
            else:
                service_data = {"device_id": await self.async_get_arg("device_id", index=self.id, default="")}
                await self.async_call_service_template("charge_stop_service", service_data)

    async def async_adjust_discharge_immediate(self, target_soc):
        """
        ...
        if self.inv_has_service_api:
            if target_soc > 0:
                ...
                await self.async_call_service_template("discharge_start_service", service_data)
            else:
                service_data = {"device_id": await self.async_get_arg("device_id", index=self.id, default="")}
                await self.async_call_service_template("charge_stop_service", service_data)

    async def async_adjust_charge_window(self, charge_start_time, charge_end_time, minutes_now):
        """
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                old_start = await self.async_get_arg("charge_start_time", index=self.id)
                old_end = await self.async_get_arg("charge_end_time", index=self.id)
                old_charge_schedule_enable = await self.async_get_arg("scheduled_charge_enable", "on", index=self.id)
            else:
        ...
        # Some inverters have an idle time setting
        await self.async_adjust_idle_time(charge_start=new_start, charge_end=new_end)
        ...
        if not in_new_window and not self.rest_data and ((new_start != old_start) or (new_end != old_end)) and self.inv_has_charge_enable_time:
            await self.async_disable_charge_window(notify=False)
            have_disabled = True
        if new_start != old_start:
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    # Always write to this as it is the GE default
                    entity_start = self.base.get_entity(await self.async_get_arg("charge_start_time", indirect=False, index=self.id))
                    if self.inv_charge_time_entity_is_option:
                        await self.async_write_and_poll_option("charge_start_time", entity_start, new_start)
                    else:
                        await self.async_write_and_poll_value("charge_start_time", entity_start, new_start)

                    if self.inv_charge_time_format == "H M":
                        # If the inverter uses hours and minutes then write to these entities too
                        entity = self.base.get_entity(await self.async_get_arg("charge_start_hour", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("charge_start_hour", entity, int(new_start[:2]))
                        entity = self.base.get_entity(await self.async_get_arg("charge_start_minute", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("charge_start_minute", entity, int(new_start[3:5]))
                else:
        ...
        if new_end != old_end:
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    # Always write to this as it is the GE default
                    entity_end = self.base.get_entity(await self.async_get_arg("charge_end_time", indirect=False, index=self.id))
                    if self.inv_charge_time_entity_is_option:
                        await self.async_write_and_poll_option("charge_end_time", entity_end, new_end)
                    else:
                        await self.async_write_and_poll_value("charge_end_time", entity_end, new_end)

                    if self.inv_charge_time_format == "H M":
                        entity = self.base.get_entity(await self.async_get_arg("charge_end_hour", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("charge_end_hour", entity, int(new_end[:2]))
                        entity = self.base.get_entity(await self.async_get_arg("charge_end_minute", indirect=False, index=self.id))
                        await self.async_write_and_poll_value("charge_end_minute", entity, int(new_end[3:5]))
                else:
        ...
        if new_start != old_start or new_end != old_end:
            if self.rest_data and not SIMULATE:
                await self.async_rest_setChargeSlot1(new_start, new_end)
            ...
            if self.base.set_inverter_notify and not SIMULATE:
                await self.base.call_notify("Predbat: Inverter {} Charge window change to: {} - {} at {}".format(self.id, new_start, new_end, self.base.time_now_str()))
            self.log("Inverter {} Updated start and end charge window to {} - {} (old {} - {})".format(self.id, new_start, new_end, old_start, old_end))

        if old_charge_schedule_enable == "off" or old_charge_schedule_enable == "disable" or have_disabled:
            if not SIMULATE:
                # Enable scheduled charge if not turned on
                if self.rest_data:
                    await self.async_rest_enableChargeSchedule(True)
                elif "scheduled_charge_enable" in self.base.args:
                    entity = self.base.get_entity(await self.async_get_arg("scheduled_charge_enable", indirect=False, index=self.id))
                    await self.async_write_and_poll_switch("scheduled_charge_enable", entity, True)
                    if not self.inv_has_charge_enable_time and (self.inv_output_charge_control == "current"):
                        await self.async_enable_charge_discharge_with_time_current("charge", True)
                else:
                ...
                if (old_charge_schedule_enable == "off" or old_charge_schedule_enable == "disable") and self.base.set_inverter_notify:
                    await self.base.call_notify("Predbat: Inverter {} Enabling scheduled charging at {}".format(self.id, self.base.time_now_str()))
            else:
            ...
            if old_charge_schedule_enable == "off" or old_charge_schedule_enable == "disable":
                self.log("Inverter {} Turning on scheduled charge".format(self.id))

    async def async_press_and_poll_button(self, entity_id):
        for retry in range(6):
            await self.base.call_service("button/press", entity_id=entity_id)
            entity = self.base.get_entity(entity_id)
            await asyncio.sleep(self.inv_write_and_poll_sleep)
            time_pressed = datetime.strptime(await entity.get_state(), TIME_FORMAT_SECONDS)

            if (pytz.timezone("UTC").localize(datetime.now()) - time_pressed).seconds < 10:
                self.log(f"Successfully pressed button {entity_id} on Inverter {self.id}")
                return True
        self.log(f"WARN: Inverter {self.id} Trying to press {entity_id} didn't complete")
        await self.base.async_record_status(f"Warn: Inverter {self.id} Trying to press {entity_id} didn't complete")
        return False

    async def async_rest_readData(self, api="readData", retry=True):
        """
        Get inverter status
        ...
        if r and (r.status_code == 200):
            ...
            if "Control" in json:
                ...
                if retry:
                    ...
                    return await self.async_rest_runAll()
                else:
                    self.log("WARN: Inverter {} read bad REST data from {} - REST will be disabled".format(self.id, url))
                    await self.base.async_record_status("Inverter {} read bad REST data from {} - REST will be disabled".format(self.id, url), had_errors=True)
                    return None
        else:
            self.log("WARN: Inverter {} unable to read REST data from {} - REST will be disabled".format(self.id, url))
            await self.base.async_record_status("Inverter {} unable to read REST data from {} - REST will be disabled".format(self.id, url), had_errors=True)
            return None

    async def async_rest_runAll(self, old_data=None):
        """
        Updated and get inverter status
        """
        new_data = await self.async_rest_readData(api="runAll", retry=False)
        if new_data:
            return new_data
        else:
            return old_data

    async def async_rest_setChargeTarget(self, target):
        """
        ...
        for retry in range(5):
            ...
            self.rest_data = await self.async_rest_runAll(self.rest_data)
            if float(self.rest_data["Control"]["Target_SOC"]) == target:
                self.log("Inverter {} charge target {} via REST successful on retry {}".format(self.id, target, retry))
                return True

        self.log("WARN: Inverter {} charge target {} via REST failed".format(self.id, target))
        await self.base.async_record_status("Warn - Inverter {} REST failed to setChargeTarget".format(self.id), had_errors=True)
        return False

    async def async_rest_setChargeRate(self, rate):
        """
        Configure charge target % via REST
        """
        rate = int(rate)
        ...
        for retry in range(5):
            ...
            if abs(new - rate) < (self.battery_rate_max_charge * MINUTE_WATT / 12):
                self.log("Inverter {} set charge rate {} via REST successful on retry {}".format(self.id, rate, retry))
                return True

        self.log("WARN: Inverter {} set charge rate {} via REST failed got {}".format(self.id, rate, self.rest_data["Control"]["Battery_Charge_Rate"]))
        await self.base.async_record_status("Warn - Inverter {} REST failed to setChargeRate".format(self.id), had_errors=True)
        return False

    async def async_rest_setDischargeRate(self, rate):
        """
        Configure charge target % via REST
        """
        rate = int(rate)
        ...
        for retry in range(5):
            ...
            if abs(new - rate) < (self.battery_rate_max_discharge * MINUTE_WATT / 25):
                self.log("Inverter {} set discharge rate {} via REST successful on retry {}".format(self.id, rate, retry))
                return True

        self.log("WARN: Inverter {} set discharge rate {} via REST failed got {}".format(self.id, rate, self.rest_data["Control"]["Battery_Discharge_Rate"]))
        await self.base.async_record_status(
    ...
    async def async_rest_setBatteryMode(self, inverter_mode):
        """
        ...
        for retry in range(5):
            ...
            if inverter_mode == self.rest_data["Control"]["Mode"]:
                self.log("Set inverter {} mode {} via REST successful on retry {}".format(self.id, inverter_mode, retry))
                return True

        self.log("WARN: Set inverter {} mode {} via REST failed".format(self.id, inverter_mode))
        await self.base.async_record_status("Warn - Inverter {} REST failed to setBatteryMode".format(self.id), had_errors=True)
        return False

    async def async_rest_setReserve(self, target):
        """
        ...
        for retry in range(5):
            ...
            if result == target:
                self.log("Set inverter {} reserve {} via REST successful on retry {}".format(self.id, target, retry))
                return True

        self.log("WARN: Set inverter {} reserve {} via REST failed on retry {} got {}".format(self.id, target, retry, result))
        await self.base.async_record_status("Warn - Inverter {} REST failed to setReserve to {} got {}".format(self.id, target, result), had_errors=True)
        return False

    async def async_rest_enableChargeSchedule(self, enable):
        """
        Configure reserve % via REST
        """
        ...
        for retry in range(5):
            ...
            if isinstance(new_value, str):
                if new_value.lower() in ["enable", "on", "true"]:
                    ...
                    new_value = False
            if new_value == enable:
                self.log("Set inverter {} charge schedule {} via REST successful on retry {}".format(self.id, enable, retry))
                return True

        self.log("WARN: Set inverter {} charge schedule {} via REST failed got {}".format(self.id, enable, self.rest_data["Control"]["Enable_Charge_Schedule"]))
        await self.base.async_record_status("Warn - Inverter {} REST failed to enableChargeSchedule".format(self.id), had_errors=True)
        return False

    async def async_rest_setChargeSlot1(self, start, finish):
        """
        ...
        for retry in range(5):
            ...
            if self.rest_data["Timeslots"]["Charge_start_time_slot_1"] == start and self.rest_data["Timeslots"]["Charge_end_time_slot_1"] == finish:
                self.log("Inverter {} set charge slot 1 {} via REST successful after retry {}".format(self.id, data, retry))
                return True

        self.log("WARN: Inverter {} set charge slot 1 {} via REST failed".format(self.id, data))
        await self.base.async_record_status("Warn - Inverter {} REST failed to setChargeSlot1".format(self.id), had_errors=True)
        return False

    async def async_rest_setDischargeSlot1(self, start, finish):
        """
        Configure charge slot via REST
        """
        ...
        for retry in range(5):
            ...
            if self.rest_data["Timeslots"]["Discharge_start_time_slot_1"] == start and self.rest_data["Timeslots"]["Discharge_end_time_slot_1"] == finish:
                self.log("Inverter {} Set discharge slot 1 {} via REST successful after retry {}".format(self.id, data, retry))
                return True

        self.log("WARN: Inverter {} Set discharge slot 1 {} via REST failed".format(self.id, data))
        await self.base.async_record_status("Warn - Inverter {} REST failed to setDischargeSlot1".format(self.id), had_errors=True)
        return False
...
class PredBat(hass.Hass):
    ...
    async def async_resolve_arg(self, arg, value, default=None, indirect=True, combine=False, attribute=None, index=None, extra_args=None):
        """
        ...
        if isinstance(value, list):
            if combine:
                final = 0
                for item in value:
                    got = await self.async_resolve_arg(arg, item, default=default, indirect=True)
                    try:
                        final += float(got)
                    except (ValueError, TypeError):
                        self.log("WARN: Return bad value {} from {} arg {}".format(got, item, arg))
                        await self.async_record_status("Warn - Return bad value {} from {} arg {}".format(got, item, arg), had_errors=True)
                return final
            else:
                final = []
                for item in value:
                    item = await self.async_resolve_arg(arg, item, default=default, indirect=indirect)
        ...
        for repeat in range(2):
            if isinstance(value, str) and "{" in value:
                try:
                    ...
                    await self.async_record_status("Warn - can not resolve {} value {}".format(arg, value), had_errors=True)
                    value = default

        # Resolve join list by name
        if isinstance(value, str) and value.startswith("+[") and value.endswith("]"):
            value = await self.async_get_arg(value[2:-1], default=default, indirect=indirect, combine=False, attribute=attribute, index=index)
        ...
        if indirect and isinstance(value, str) and "." in value:
            ...
            if attribute:
                value = await self.get_state(entity_id=value, default=default, attribute=attribute)
            else:
                value = await self.get_state(entity_id=value, default=default)
        return value

    async def async_get_arg(self, arg, default=None, indirect=True, combine=False, attribute=None, index=None):
        """
        ...
        # Get From HA config
        value, default = await self.async_get_ha_config(arg, default)
        ...
        if value is None:
            ...
            value = await self.async_resolve_arg(arg, value, default=default, indirect=indirect, combine=combine, attribute=attribute, index=index)
        ...
        if isinstance(default, float):
            ...
            try:
                ...
                await self.async_record_status("Warn - Return bad float value {} from {}".format(value, arg), had_errors=True)
                value = default
            ...
            try:
                ...
                await self.async_record_status("Warn - Return bad int value {} from {}".format(value, arg), had_errors=True)
                value = default
    ...
    async def async_get_ge_url(self, url, headers, now_utc):
        """
        ...
        try:
            ...
            await self.async_record_status("Warn - Error downloading GE data from cloud", debug=url, had_errors=True)
            return False
    ...
    async def async_download_ge_data(self, now_utc):
        """
        Download consumption data from GE Cloud
        """
        geserial = await self.async_get_arg("ge_cloud_serial")
        ...
        if not geserial:
            ...
            await self.async_record_status("Warn - GE Cloud has been enabled but ge_cloud_serial is not set to your serial", had_errors=True)
            return False
        if not gekey:
            self.log("ERROR: GE Cloud has been enabled but ge_cloud_key is not set to your appkey")
            await self.async_record_status("Warn - GE Cloud has been enabled but ge_cloud_key is not set to your appkey", had_errors=True)
            return False
        ...
        while days_prev <= self.max_days_previous:
            ...
            while url:
                data = await self.async_get_ge_url(url, headers, now_utc)

                darray = data.get("data", None)
                if darray is None:
                    self.log("WARN: Error downloading GE data from url {}".format(url))
                    await self.async_record_status("Warn - Error downloading GE data from cloud", debug=url)
                    return False
        ...
        item = mdata[0]
        try:
            last_updated_time = await self.async_str2time(item["last_updated"])
        except (ValueError, TypeError):
            last_updated_time = now_utc

        age = now_utc - last_updated_time
        self.load_minutes_age = age.days
        self.load_minutes = await self.async_minute_data(
        ...
        self.import_today = await self.async_minute_data(
        ...
        self.export_today = await self.async_minute_data(
        ...
        self.pv_today = await self.async_minute_data(
    ...
    async def async_download_predbat_releases_url(self, url):
        """
    ...
    async def async_download_predbat_releases(self):
        """
        Download release data
        """
        global PREDBAT_UPDATE_OPTIONS
        auto_update = await self.async_get_arg("auto_update")
        url = "https://api.github.com/repos/springfall2008/batpred/releases"
        data = await self.async_download_predbat_releases_url(url)
        self.releases = {}
            ...
            if this_tag not in PREDBAT_UPDATE_OPTIONS:
                ...
                if self.releases["this"] == self.releases["latest"]:
                    ...
                    if auto_update:
                        ...
                        await self.async_download_predbat_version(latest_version)
                    else:
                        self.log("Autoupdate: There is an update pending {} - auto update is off".format(latest_version))
                        new_version = True

            # Refresh the list
            await self.async_expose_config("update", this_tag)
            await self.async_expose_config("version", new_version, force=True)

        else:
            self.log("WARN: Unable to download Predbat version information from github, return code: {}".format(data))
            await self.async_expose_config("version", False, force=True)

        return self.releases

    async def async_download_octopus_rates(self, url):
        """
        ...
        for retry in range(3):
            pdata = await self.async_download_octopus_rates_func(url)
            if pdata:
                break
        ...
        if not pdata:
            ...
            await self.async_record_status("Warn - Unable to download Octopus data from cloud", debug=url, had_errors=True)
            if url in self.octopus_url_cache:
                pdata = self.octopus_url_cache[url]["data"]
                return pdata
            else:
    ...
    async def async_futurerate_analysis(self):
        """
        ...
        if "futurerate_url" in self.args:
            url = await self.async_get_arg("futurerate_url", indirect=False)
        self.log("Fetching futurerate data from {}".format(url))
        if not url:
            return {}, {}

        try:
            pdata = await self.async_download_futurerate_data(url)
        except ValueError:
            return {}, {}
        ...
        peak_start = datetime.strptime(await self.async_get_arg("futurerate_peak_start", "00:00:00"), "%H:%M:%S")
        peak_end = datetime.strptime(await self.async_get_arg("futurerate_peak_end", "00:00:00"), "%H:%M:%S")
        ...
        peak_premium_import = await self.async_get_arg("futurerate_peak_premium_import", 0)
        peak_premium_export = await self.async_get_arg("futurerate_peak_premium_export", 0)
        ...
        if extracted_keys:
            ...
            mdata_import = await self.async_minute_data(array_values, self.forecast_days + 1, self.midnight_utc, "rate_import", "from", backwards=False, to_key="to")
            mdata_export = await self.async_minute_data(array_values, self.forecast_days + 1, self.midnight_utc, "rate_export", "from", backwards=False, to_key="to")
    ...
    async def async_download_futurerate_data(self, url):
        """
        ...
        # Retry up to 3 minutes
        for retry in range(3):
            pdata = await self.async_download_futurerate_data_func(url)
            if pdata:
                break
        ...
        if not pdata:
            ...
            await self.async_record_status("Warn - Unable to download futurerate data from cloud", debug=url, had_errors=True)
            if url in self.octopus_url_cache:
                pdata = self.futurerate_url_cache[url]["data"]
                return pdata
            else:
    ...
    async def async_download_futurerate_data_func(self, url):
        """
        ...
        try:
            ...
            await self.async_record_status("Warn - Error downloading futurerate data from cloud", debug=url, had_errors=True)
            return {}
        if "data" in data:
            mdata = data["data"]
        else:
    ...
    async def async_download_octopus_rates_func(self, url):
        """
        Download octopus rates directly from a URL
        """
        mdata = []
        ...
        while url and pages < 3:
            ...
            try:
                ...
                await self.async_record_status("Warn - Error downloading Octopus data from cloud", debug=url, had_errors=True)
                return {}
            if "results" in data:
                mdata += data["results"]
            else:
            ...
            pages += 1
        pdata = await self.async_minute_data(mdata, self.forecast_days + 1, self.midnight_utc, "value_inc_vat", "valid_from", backwards=False, to_key="valid_to")
        return pdata

    async def async_minutes_to_time(self, updated, now):
        """
    ...
    async def async_str2time(self, str):
        if "." in str:
    ...
    async def async_load_car_energy(self, now_utc):
        """
        Load previous car charging energy data
        """
        self.car_charging_energy = {}
        if "car_charging_energy" in self.args:
            self.car_charging_energy = await self.async_minute_data_import_export(now_utc, "car_charging_energy", scale=self.car_charging_energy_scale, required_unit="kWh")
        else:
    ...
    async def async_get_history_async(self, entity_id, days=None):
        """
        Async function to get history from HA using Async task
        """
        result = await self.get_history_async_hook(entity_id, days)
    ...
    async def async_minute_data_import_export(self, now_utc, key, scale=1.0, required_unit=None, increment=True, smoothing=True):
        """
        Download one or more entities for import/export data
        """
        if "." not in key:
            entity_ids = await self.async_get_arg(key, indirect=False)
        else:
            entity_ids = key
        ...
        for entity_id in entity_ids:
            try:
                history = await self.async_get_history_async(entity_id=entity_id, days=self.max_days_previous)
            except (ValueError, TypeError):
                history = []

            if history:
                import_today = await self.async_minute_data(
                    history[0],
                ...
                await self.async_record_status("Error - Unable to fetch history from {}".format(entity_id), had_errors=True)
                raise ValueError

        return import_today

    async def async_minute_data_load(self, now_utc, entity_name, max_days_previous, required_unit=None):
        """
        Download one or more entities for load data
        """
        entity_ids = await self.async_get_arg(entity_name, indirect=False)
        if isinstance(entity_ids, str):
            entity_ids = [entity_ids]
        ...
        age_days = None
        for entity_id in entity_ids:
            history = await self.async_get_history_async(entity_id=entity_id, days=max_days_previous)
            if history:
                item = history[0][0]
                try:
                    last_updated_time = await self.async_str2time(item["last_updated"])
                ...
                load_minutes = await self.async_minute_data(
                    history[0],
                    max_days_previous,
                    now_utc,
                    "state",
    ...
    async def async_minute_data_state(self, history, days, now, state_key, last_updated_key):
        """
        ...
        for item in history:
            ...
            last_updated_time = await self.async_str2time(item[last_updated_key])
        ...
        for item in history:
            ...
            if to_key:
                ...
                if not to_value:
                    ...
                    to_time = await self.async_str2time(item[to_key])
            else:
                if backwards:
    ...
    async def async_get_filtered_load_minute(self, data, minute_previous, historical, step=1):
        """
        ...
        for offset in range(step):
            if historical:
                load_yesterday_raw += await self.async_get_historical(data, minute_previous + offset)
            else:
                load_yesterday_raw += await self.async_get_from_incrementing(data, minute_previous + offset)
        ...
        if self.car_charging_hold and self.car_charging_energy:
            ...
            for offset in range(step):
                if historical:
                    car_energy += await self.async_get_historical(self.car_charging_energy, minute_previous + offset)
                else:
                    car_energy += await self.async_get_from_incrementing(self.car_charging_energy, minute_previous + offset)
    ...
    async def async_previous_days_modal_filter(self, data):
        """
        ...
        for days in self.days_previous:
            ...
            for minute in range(0, 24 * 60, PREDICT_STEP):
                ...
                load_yesterday, load_yesterday_raw = await self.async_get_filtered_load_minute(data, minute_previous, historical=False, step=PREDICT_STEP)
        ...
        # Gap filling
        gap_size = max(await self.async_get_arg("load_filter_threshold", 30), 5)
        for days in self.days_previous:
            use_days = max(min(days, self.load_minutes_age), 1)
            num_gaps = 0
            full_days = 24 * 60 * (use_days - 1)
    ...
    async def async_get_historical(self, data, minute):
        """
        ...
        for days in self.days_previous:
            ...
            full_days = 24 * 60 * (use_days - 1)
            minute_previous = 24 * 60 - minute + full_days
            value = await self.async_get_from_incrementing(data, minute_previous)
            total += value * weight
            total_weight += weight
            this_point += 1
    ...
    async def async_get_from_incrementing(self, data, index, backwards=True):
        """
    ...
    async def async_record_length(self, charge_window, charge_limit, best_price):
        """
        ...
        max_windows = await self.async_max_charge_windows(end_record, charge_window)
    ...
    async def async_max_charge_windows(self, end_record_abs, charge_window):
        """
    ...
    async def async_record_status(self, message, debug="", had_errors=False, notify=False, extra=""):
        """
        ...
        if notify and self.previous_status != message and self.set_status_notify:
            await self.async_call_notify("Predbat status change to: " + message + extra)
    ...
    async def async_load_today_comparison(self, load_minutes, load_forecast, car_minutes, import_minutes, minutes_now, step=5):
        """
        ...
        for minute in range(0, 24 * 60, step):
            ...
            if minute < minutes_now:
                for offset in range(step):
                    import_value_today += await self.async_get_from_incrementing(import_minutes, minutes_now - minute - offset - 1)
                    load_value_today, load_value_today_raw = await self.async_get_filtered_load_minute(load_minutes, minutes_now - minute - 1, historical=False, step=step)

            import_value_pred = 0
            forecast_value_pred = 0
            for offset in range(step):
                import_value_pred += await self.async_get_historical(import_minutes, minute - minutes_now + offset)
                forecast_value_pred += await self.async_get_from_incrementing(load_forecast, minute + offset, backwards=False)

            if self.load_forecast_only:
                load_value_pred, load_value_pred_raw = (0, 0)
            else:
                load_value_pred, load_value_pred_raw = await self.async_get_filtered_load_minute(load_minutes, minute - minutes_now, historical=True, step=step)
        ...
        await self.async_dashboard_item(
            ...
            attributes={
                "results": await self.async_filtered_times(load_actual_stamp),
        ...
        await self.async_dashboard_item(
            ...
            attributes={
                "results": await self.async_filtered_times(load_predict_stamp),
                "today": await self.async_filtered_today(load_predict_stamp),
        ...
        await self.async_dashboard_item(
            ...
            attributes={
                "results": await self.async_filtered_times(load_adjusted_stamp),
                "today": await self.async_filtered_today(load_adjusted_stamp),
    ...
    async def async_get_load_divergence(self, minutes_now, load_minutes):
        """
        ...
        for minute in range(0, look_over, PREDICT_STEP):
            load, load_raw = await self.async_get_filtered_load_minute(load_minutes, minute, historical=True, step=PREDICT_STEP)
    ...
    async def async_get_cloud_factor(self, minutes_now, pv_data, pv_data10):
        """
    ...
    async def async_step_data_history(
        ...
        for minute in range(0, self.forecast_minutes, step):
            ...
            if type_load and not forward:
                if self.load_forecast_only:
                    ...
                    load_yesterday, load_yesterday_raw = await self.async_get_filtered_load_minute(item, minute, historical=True, step=step)
                ...
                for offset in range(step):
                    if forward:
                        ...
                        value += await self.async_get_historical(item, minute + offset)

            # Extra load adding in (e.g. heat pump)
            load_extra = 0
            if load_forecast:
                for offset in range(step):
                    load_extra += await self.async_get_from_incrementing(load_forecast, minute_absolute, backwards=False)
    ...
    async def async_filtered_today(self, time_data):
        """
    ...
    async def async_filtered_times(self, time_data):
        """
    ...
    async def async_run_prediction(self, charge_limit, charge_window, discharge_window, discharge_limits, pv10, end_record, save=None, step=PREDICT_STEP):
        """
        ...
        if save or self.debug_enable:
            ...
            if save and save == "base" and not SIMULATE:
                ...
                for car_n in range(self.num_cars):
                    ...
                    await self.async_dashboard_item(
                        ...
                        attributes={
                            "results": await self.async_filtered_times(predict_car_soc_time[car_n]),
                            "today": await self.async_filtered_today(predict_car_soc_time[car_n]),
                ...
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(predict_soc_time),
                        "today": await self.async_filtered_today(predict_soc_time),
                ...
                await self.async_dashboard_item(
                    ...
                    state=self.dp3(0),
                    attributes={
                        "results": await self.async_filtered_times(predict_battery_power),
                        "today": await self.async_filtered_today(predict_battery_power),
                ...
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(predict_battery_cycle),
                        "today": await self.async_filtered_today(predict_battery_cycle),
                ...
                await self.async_dashboard_item(
                    ...
                    state=self.dp3(0),
                    attributes={
                        "results": await self.async_filtered_times(predict_pv_power),
                        "today": await self.async_filtered_today(predict_pv_power),
                ...
                await self.async_dashboard_item(
                    ...
                    state=self.dp3(final_soc),
                    attributes={
                        "results": await self.async_filtered_times(predict_grid_power),
                        "today": await self.async_filtered_today(predict_grid_power),
                ...
                await self.async_dashboard_item(
                    ...
                    state=self.dp3(final_soc),
                    attributes={
                        "results": await self.async_filtered_times(predict_load_power),
                        "today": await self.async_filtered_today(predict_load_power),
                ...
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(export_kwh_time),
                        "today": await self.async_filtered_today(export_kwh_time),
                ...
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(load_kwh_time),
                        "today": await self.async_filtered_today(load_kwh_time),
                ...
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(import_kwh_time),
                        "today": await self.async_filtered_today(import_kwh_time),
                ...
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(metric_time),
                        "today": await self.async_filtered_today(metric_time),
                ...
                if self.carbon_enable:
                    await self.async_dashboard_item(
                        ...
                        attributes={
                            "results": await self.async_filtered_times(predict_carbon_g),
                            "today": await self.async_filtered_today(predict_carbon_g),
            ...
            if save and save == "best" and not SIMULATE:
                ...
                await self.async_dashboard_item(
                    ...
                    state=self.dp3(final_pv_kwh),
                    attributes={
                        "results": await self.async_filtered_times(pv_kwh_time),
                        "today": await self.async_filtered_today(pv_kwh_time),
                ...
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(predict_iboost),
                        "today": await self.async_filtered_today(predict_iboost),
                ...
                await self.async_find_spare_energy(self.predict_soc, predict_export, step, first_charge)
                if self.carbon_enable:
                    await self.async_dashboard_item(
    ...
    async def async_rate_replicate(self, rates, rate_io={}, is_import=True, is_gas=False):
        """
        ...
        while minute < (self.forecast_minutes + 48 * 60):
            if minute not in rates:
                ...
                if minute_mod not in adjusted_rates:
                    if (
                        is_import
                        and await self.async_get_arg("futurerate_adjust_import", False)
                        ...
                        (not is_import)
                        and (not is_gas)
                        and await self.async_get_arg("futurerate_adjust_export", False)
    ...
    async def async_find_charge_window(self, rates, minute, threshold_rate, find_high):
        """
    ...
    async def async_basic_rates(self, info, rtype, prev=None, rate_replicate={}):
        """
        ...
        for this_rate in info:
            if this_rate:
                ...
                start_str = await self.async_resolve_arg("start", start_str, "00:00:00")
                end_str = this_rate.get("end", "00:00:00")
                end_str = await self.async_resolve_arg("end", end_str, "00:00:00")
                ...
                try:
                    ...
                    await self.async_record_status("Bad start time {} provided in energy rates".format(start_str), had_errors=True)
                    continue
                ...
                try:
                    ...
                    await self.async_record_status("Bad end time {} provided in energy rates".format(end_str), had_errors=True)
                    continue

                date = None
                if "date" in this_rate:
                    date_str = await self.async_resolve_arg("date", this_rate["date"])
                    try:
                        date = datetime.strptime(date_str, "%Y-%m-%d")
                    except ValueError:
                        self.log("WARN: Bad date {} provided in energy rates".format(this_rate["date"]))
                        await self.async_record_status("Bad date {} provided in energy rates".format(this_rate["date"]), had_errors=True)
                        continue
                ...
                rate = await self.async_resolve_arg("rate", rate, 0.0)
                ...
                try:
                    ...
                    await self.async_record_status("Bad rate {} provided in energy rates".format(rate), had_errors=True)
                    continue

                # Time in minutes
                start_minutes = max(await self.async_minutes_to_time(start, midnight), 0)
                end_minutes = min(await self.async_minutes_to_time(end, midnight), 24 * 60 - 1)
                ...
                if date:
                    delta_minutes = await self.async_minutes_to_time(date, self.midnight)
    ...
    async def async_plan_car_charging(self, car_n, low_rates):
        """
        ...
        if self.car_charging_plan_smart[car_n]:
            price_sorted = await self.async_sort_window_by_price(low_rates)
        ...
        plan = await self.async_sort_window_by_time(plan)
        return plan

    async def async_add_now_to_octopus_slot(self, octopus_slots, now_utc):
        """
    ...
    async def async_load_saving_slot(self, octopus_saving_slots, export=False, rate_replicate={}):
        """
        ...
        for octopus_saving_slot in octopus_saving_slots:
            ...
            if start and end:
                try:
                    start = await self.async_str2time(start)
                    end = await self.async_str2time(end)
                except ValueError:
                    start = None
                    end = None
            ...
            if state and (not start or not end):
                ...
                start_minutes = await self.async_minutes_to_time(start, self.midnight_utc)
                end_minutes = min(await self.async_minutes_to_time(end, self.midnight_utc), self.forecast_minutes)
    ...
    async def async_decode_octopus_slot(self, slot):
        """
        Decode IOG slot
        """
        if "start" in slot:
        ...
        start_minutes = await self.async_minutes_to_time(start, self.midnight_utc)
        end_minutes = await self.async_minutes_to_time(end, self.midnight_utc)
    ...
    async def async_load_octopus_slots(self, octopus_slots):
        """
        ...
        for slot in octopus_slots:
            start_minutes, end_minutes, kwh, source, location = await self.async_decode_octopus_slot(slot)
    ...
    async def async_rate_scan_export(self, rates, print=True):
        """
        Scan the rates and work out min/max
        """

        self.rate_export_min, self.rate_export_max, self.rate_export_average, self.rate_export_min_minute, self.rate_export_max_minute = await self.async_rate_minmax(rates)
        if print:
            self.log("Export rates min {} max {} average {}".format(self.rate_export_min, self.rate_export_max, self.rate_export_average))
        return rates

    async def async_publish_car_plan(self):
        """
    ...
    async def async_publish_rates_export(self):
        """
    ...
    async def async_rate_minmax(self, rates):
        """
    ...
    async def async_rate_min_forward_calc(self, rates):
        """
    ...
    async def async_rate_scan_window(self, rates, rate_low_min_window, threshold_rate, find_high):
        """
        ...
        while True:
            rate_low_start, rate_low_end, rate_low_average = await self.async_find_charge_window(rates, minute, threshold_rate, find_high)
            window = {}
        ...
        selected_rates = []
        total = 0
        window_sorted, window_index, price_set, price_links = await self.async_sort_window_by_price_combined(found_rates, [], stand_alone=True)
        if not find_high:
            price_set.reverse()
    ...
    async def async_set_rate_thresholds(self):
        """
    ...
    async def async_rate_add_io_slots(self, rates, octopus_slots):
        """
    ...
    async def async_rate_scan(self, rates, print=True):
        """
        Scan the rates and work out min/max
        """
        self.low_rates = []

        self.rate_min, self.rate_max, self.rate_average, self.rate_min_minute, self.rate_max_minute = await self.async_rate_minmax(rates)

        if print:
            # Calculate minimum forward rates only once rate replicate has run (when print is True)
            self.rate_min_forward = await self.async_rate_min_forward_calc(self.rate_import)
            self.log("Import rates min {} max {} average {}".format(self.rate_min, self.rate_max, self.rate_average))

        return rates

    async def async_rate_scan_gas(self, rates, print=True):
        """
        Scan the gas rates and work out min/max
        """
        self.rate_gas_min, self.rate_gas_max, self.rate_gas_average, self.rate_gas_min_minute, self.rate_gas_max_minute = await self.async_rate_minmax(rates)
    ...
    async def async_publish_rates_import(self):
        """
    ...
    async def async_adjust_symbol(self, adjust_type):
        """
    ...
    async def async_car_charge_slot_kwh(self, minute):
        """
    ...
    async def async_hit_car_window(self, window_start, window_end):
        """
    ...
    async def async_get_html_plan_header(self, plan_debug):
        """
    ...
    async def async_publish_html_plan(self, pv_forecast_minute_step, pv_forecast_minute_step10, load_minutes_step, load_minutes_step10, end_record):
        """
        Publish the current plan in HTML format
        """
        plan_debug = await self.async_get_arg("plan_debug")
        html = "<table>"
        html += "<tr>"
        ...
        html += "</tr>"
        html += await self.async_get_html_plan_header(plan_debug)
        ...
        for minute in range(minute_now_align, end_plan, 30):
            ...
            adjust_symbol = await self.async_adjust_symbol(adjust_type)
            if adjust_symbol:
            ...
            # Car charging?
            if self.num_cars > 0:
                car_charging_kwh = await self.async_car_charge_slot_kwh(minute)
            ...
            html += "</tr>"
        html += "</table>"
        await self.async_dashboard_item(self.prefix + ".plan_html", state="", attributes={"html": html, "friendly_name": "Plan in HTML", "icon": "mdi:web-box"})

    async def async_publish_rates(self, rates, export, gas=False):
        """
        ...
        if export:
            await self.async_publish_rates_export()
        elif gas:
            pass
        else:
            await self.async_publish_rates_import()
            ...
            if export:
                await self.async_dashboard_item(
                    ...
                    attributes={
                        ...
                        "results": await self.async_filtered_times(rates_time),
    ...
    async def async_today_cost(self, import_today, export_today):
        """
        ...
        for minute in range(self.minutes_now):
            ...
            energy = 0
            energy = await self.async_get_from_incrementing(import_today, minute_back)
            if export_today:
                energy_export = await self.async_get_from_incrementing(export_today, minute_back)
            else:
                energy_export = 0
            day_energy += energy
        ...
        if not SIMULATE:
            await self.async_dashboard_item(
                ...
                attributes={
                    "results": await self.async_filtered_times(day_cost_time),
            ...
            if self.carbon_enable:
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(day_carbon_time),
            ...
            await self.async_dashboard_item(
                ...
                attributes={
                    "results": await self.async_filtered_times(day_cost_time_import),
            ...
            await self.async_dashboard_item(
                ...
                attributes={
                    "results": await self.async_filtered_times(day_cost_time_export),
    ...
    async def async_publish_discharge_limit(self, discharge_window, discharge_limits, best):
        """
        ...
        if not SIMULATE:
            ...
            if best:
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(discharge_limit_time_kw),
                ...
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(discharge_limit_time),
    ...
    async def async_publish_charge_limit(self, charge_limit, charge_window, charge_limit_percent, best=False, soc={}):
        """
        ...
        if not SIMULATE:
            ...
            if best:
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(charge_limit_time_kw),
                ...
                await self.async_dashboard_item(
                    ...
                    attributes={
                        "results": await self.async_filtered_times(charge_limit_time),
    ...
    async def async_reset(self):
        ...
        self.pool = None
        self.run_time_task = None
        self.update_time_task = None
        self.balance_time_task = None
    ...
    async def async_optimise_charge_limit_price(
        self,
        price_set,
        price_links,
        window_index,
                ...
                for divide in [1, 2, 3, 4, 8, 16, 32, 96]:
                    ...
                    for discharge_enable in discharge_enable_options:
                        ...
                        (
                            ...
                            final_iboost,
                            final_carbon_g,
                        ) = await self.async_run_prediction(
                            try_charge_limit,
                            charge_window,
                            discharge_window,
                            try_discharge,
                        ...
                        metric, metric10 = await self.async_compute_metric(
    ...
    async def async_launch_run_prediction_charge(self, loop_soc, window_n, charge_limit, charge_window, discharge_window, discharge_limits, pv10, all_n, end_record):
        """
    ...
    async def async_optimise_charge_limit_swap(self, charge_limit, charge_window, discharge_window, discharge_limits, record_charge_windows, end_record=None):
        """
        ...
        price_sorted = await self.async_sort_window_by_price(charge_window)
        price_sorted.reverse()
    ...
    async def async_compute_metric(
    ...
    async def async_optimise_charge_limit(self, window_n, record_charge_windows, charge_limit, charge_window, discharge_window, discharge_limits, all_n=None, end_record=None):
        """
    ...
    async def async_optimise_discharge(
        ...
        for loop_limit in loop_options:
            ...
            while loop_start >= window["start"]:
                ...
                results.append(
                    await self.async_launch_run_prediction_discharge(
    ...
    async def async_window_sort_func(self, window):
        """
        Helper sort index function
        """
        return float(window["key"])

    async def async_window_sort_func_start(self, window):
        """
        Helper sort index function
        """
        return float(window["start"])

    async def async_sort_window_by_time(self, windows):
        """
    ...
    async def async_sort_window_by_price_combined(self, charge_windows, discharge_windows, stand_alone=False, secondary_order=False):
        """
    ...
    async def async_sort_window_by_time_combined(self, charge_windows, discharge_windows):
        window_sort = []
        window_links = {}
    ...
    async def async_sort_window_by_price(self, windows, reverse_time=False):
        """
    ...
    async def async_discard_unused_charge_slots(self, charge_limit_best, charge_window_best, reserve):
        """
    ...
    async def async_find_spare_energy(self, predict_soc, predict_export, step, first_charge):
        """
        ...
        for trigger in triggers:
            ...
            try:
                ...
                await self.async_record_status("ERROR: Bad energy value {} provided via trigger {}".format(energy, name), had_errors=True)
    ...
    async def async_set_charge_discharge_status(self, isCharging, isDischarging):
        """
    ...
    async def async_clip_charge_slots(self, minutes_now, predict_soc, charge_window_best, charge_limit_best, record_charge_windows, step):
        """
    ...
    async def async_clip_discharge_slots(self, minutes_now, predict_soc, discharge_window_best, discharge_limits_best, record_discharge_windows, step):
        """
    ...
    async def async_discard_unused_discharge_slots(self, discharge_limits_best, discharge_window_best):
        """
    ...
    async def async_tweak_plan(self, end_record, best_metric, metric_keep):
        """
        Tweak existing plan only
        """
        record_charge_windows = max(await self.async_max_charge_windows(end_record + self.minutes_now, self.charge_window_best), 1)
        record_discharge_windows = max(await self.async_max_charge_windows(end_record + self.minutes_now, self.discharge_window_best), 1)
        ...
        for key in window_sorted:
            ...
            if typ == "c":
                ...
                if self.calculate_best_charge and (window_start not in self.manual_all_times):
                    best_soc, best_metric, best_cost, soc_min, soc_min_minute, best_keep, best_cycle, best_carbon = await self.async_optimise_charge_limit(
                        window_n,
                        record_charge_windows,
                        self.charge_limit_best,
                        self.charge_window_best,
    ...
    async def async_optimise_all_windows(self, best_metric, metric_keep):
        """
        Optimise all windows, both charge and discharge in rate order
        """
        record_charge_windows = max(await self.async_max_charge_windows(self.end_record + self.minutes_now, self.charge_window_best), 1)
        record_discharge_windows = max(await self.async_max_charge_windows(self.end_record + self.minutes_now, self.discharge_window_best), 1)
        window_sorted, window_index, price_set, price_links = await self.async_sort_window_by_price_combined(
            self.charge_window_best[:record_charge_windows], self.discharge_window_best[:record_discharge_windows]
        )
        best_soc = self.soc_max
        best_cost = best_metric
        ...
        if price_set and self.calculate_best_charge and self.charge_window_best:
            ...
            await self.async_optimise_charge_windows_reset(reset_all=True)
            await self.async_optimise_charge_windows_manual()
            (
                self.charge_limit_best,
                ...
                best_cycle,
                best_carbon,
            ) = await self.async_optimise_charge_limit_price(
                price_set,
                price_links,
                window_index,
                record_charge_windows,
            ...
            if self.calculate_regions:
                self.end_record = await self.async_record_length(self.charge_window_best, self.charge_limit_best, best_price)
        ...
        self.end_record = await self.async_record_length(self.charge_window_best, self.charge_limit_best, best_price)
        await self.async_optimise_charge_windows_reset(reset_all=False)
        await self.async_optimise_charge_windows_manual()
        record_charge_windows = max(await self.async_max_charge_windows(self.end_record + self.minutes_now, self.charge_window_best), 1)
        record_discharge_windows = max(await self.async_max_charge_windows(self.end_record + self.minutes_now, self.discharge_window_best), 1)
        window_sorted, window_index, price_set, price_links = await self.async_sort_window_by_price_combined(
    ...
    async def async_optimise_charge_windows_manual(self):
        """
    ...
    async def async_optimise_charge_windows_reset(self, reset_all):
        """
    ...
    async def async_get_car_charging_planned(self):
        """
        ...
        self.car_charging_planned_response = await self.async_get_arg("car_charging_planned_response", ["yes", "on", "enable", "true"])
        self.car_charging_now_response = await self.async_get_arg("car_charging_now_response", ["yes", "on", "enable", "true"])
        self.car_charging_from_battery = await self.async_get_arg("car_charging_from_battery")

        # Car charging planned sensor
        for car_n in range(self.num_cars):
            # Get car N planned status
            planned = await self.async_get_arg("car_charging_planned", "no", index=car_n)
            ...
            charging_now = await self.async_get_arg("car_charging_now", "no", index=car_n)
            ...
            self.car_charging_plan_smart[car_n] = await self.async_get_arg("car_charging_plan_smart", False)
            self.car_charging_plan_max_price[car_n] = await self.async_get_arg("car_charging_plan_max_price", 0.0)
            self.car_charging_plan_time[car_n] = await self.async_get_arg("car_charging_plan_time", "07:00:00")
            self.car_charging_battery_size[car_n] = float(await self.async_get_arg("car_charging_battery_size", 100.0, index=car_n))
            self.car_charging_rate[car_n] = float(await self.async_get_arg("car_charging_rate"))
            self.car_charging_limit[car_n] = self.dp3((float(await self.async_get_arg("car_charging_limit", 100.0, index=car_n)) * self.car_charging_battery_size[car_n]) / 100.0)
    ...
    async def async_fetch_pv_datapoints(self, argname):
        """
        ...
        if argname in self.args:
            ...
            entity_id = await self.async_get_arg(argname, None, indirect=False)
            if entity_id:
                result = await self.get_state(entity_id=entity_id, attribute=attribute)
                if not result:
                    attribute = "forecast"
                try:
                    data = await self.get_state(entity_id=await self.async_get_arg(argname, indirect=False), attribute=attribute)
                except (ValueError, TypeError):
                    self.log("WARN: Unable to fetch solar forecast data from sensor {} check your setting of {}".format(await self.async_get_arg(argname, indirect=False), argname))
                    await self.async_record_status("Error - {} not be set correctly, check apps.yaml", debug=await self.async_get_arg(argname, indirect=False), had_errors=True)
            ...
            if data:
                ...
                total_sensor = self.dp2(await self.async_get_arg(argname, 1.0))
        return data, total_data, total_sensor

    async def async_fetch_extra_load_forecast(self, now_utc):
        """
        Fetch extra load forecast, this is future load data
        """
        load_forecast = {}
        if "load_forecast" in self.args:
            entity_ids = await self.async_get_arg("load_forecast", indirect=False)
            if isinstance(entity_ids, str):
                entity_ids = [entity_ids]
            ...
            for entity_id in entity_ids:
                ...
                try:
                    ...
                    data = await self.get_state(entity_id=entity_id, attribute=attribute)
                ...
                # Load data
                load_forecast = await self.async_minute_data(
                    data,
                    self.forecast_days,
                    self.midnight_utc,
                    "energy",
    ...
    async def async_fetch_pv_forecast(self):
        """
        ...
        for argname in ["pv_forecast_today", "pv_forecast_tomorrow", "pv_forecast_d3", "pv_forecast_d4"]:
            data, total_data, total_sensor = await self.async_fetch_pv_datapoints(argname)
            if data:
        ...
        if pv_forecast_data:
            pv_estimate = await self.async_get_arg("pv_estimate", default="")
            if pv_estimate is None:
                pv_estimate = "pv_estimate"
            else:
                pv_estimate = "pv_estimate" + str(pv_estimate)

            pv_forecast_minute = await self.async_minute_data(
                pv_forecast_data,
            ...
            pv_forecast_minute10 = await self.async_minute_data(
                pv_forecast_data,
                self.forecast_days + 1,
                self.midnight_utc,
                "pv_estimate10",
    ...
    async def async_balance_inverters(self):
        """
        ...
        await self.async_update_time(print=False)

        # For each inverter get the details
        num_inverters = int(await self.async_get_arg("num_inverters", 1))

        inverters = []
        for id in range(num_inverters):
            inverter = Inverter(self, id, quiet=True)
            await inverter.async_init_inverter()
            await inverter.async_update_status(self.minutes_now, quiet=True)
    ...
    async def async_log_option_best(self):
        """
        Log options
        """
        opts = ""
    ...
    async def async_calculate_plan(self, recompute=True):
        """
        ...
        self.metric_cloud_coverage = await self.async_get_cloud_factor(self.minutes_now, self.pv_forecast_minute, self.pv_forecast_minute10)
        self.metric_load_divergence = await self.async_get_load_divergence(self.minutes_now, self.load_minutes)
        load_minutes_step = await self.async_step_data_history(
            self.load_minutes,
            self.minutes_now,
            forward=False,
        ...
        load_minutes_step10 = await self.async_step_data_history(
            self.load_minutes,
            self.minutes_now,
            forward=False,
            scale_today=self.load_inday_adjustment,
        ...
        pv_forecast_minute_step = await self.async_step_data_history(self.pv_forecast_minute, self.minutes_now, forward=True, cloud_factor=self.metric_cloud_coverage)
        pv_forecast_minute10_step = await self.async_step_data_history(
        ...
        # Create pool
        if not self.pool:
            threads = await self.async_get_arg("threads", "auto")
            if threads == "auto":
        ...
        if self.calculate_best and recompute:
            ...
            await self.async_log_option_best()

            # Full plan
            await self.async_optimise_all_windows(metric, metric_keep)

            # Tweak plan
            if self.calculate_tweak_plan:
                await self.async_tweak_plan(self.end_record, metric, metric_keep)
            ...
            if self.calculate_best_discharge and self.discharge_window_best:
                # Filter out the windows we disabled
                self.discharge_limits_best, self.discharge_window_best = await self.async_discard_unused_discharge_slots(self.discharge_limits_best, self.discharge_window_best)
                ...
                if self.discharge_window_best:
                    ...
                    self.discharge_window_best, self.discharge_limits_best = await self.async_clip_discharge_slots(
            ...
            if self.calculate_best_charge and self.charge_window_best:
                ...
                if self.set_charge_window:
                    record_charge_windows = max(await self.async_max_charge_windows(self.end_record + self.minutes_now, self.charge_window_best), 1)
                    self.charge_limit_best, self.charge_window_best = await self.async_discard_unused_charge_slots(self.charge_limit_best, self.charge_window_best, self.reserve)

                # Charge slot clipping
                record_charge_windows = max(await self.async_max_charge_windows(self.end_record + self.minutes_now, self.charge_window_best), 1)
                self.charge_window_best, self.charge_limit_best = await self.async_clip_charge_slots(
        ...
        if self.calculate_best:
            ...
            self.charge_limit_percent_best = calc_percent_limit(self.charge_limit_best, self.soc_max)
            await self.async_publish_charge_limit(self.charge_limit_best, self.charge_window_best, self.charge_limit_percent_best, best=True, soc=self.predict_soc_best)
            await self.async_publish_discharge_limit(self.discharge_window_best, self.discharge_limits_best, best=True)

            # HTML data
            await self.async_publish_html_plan(pv_forecast_minute_step, pv_forecast_minute10_step, load_minutes_step, load_minutes_step10, self.end_record)
    ...
    async def async_reset_inverter(self):
        """
        ...
        if not self.set_read_only or (self.inverter_needs_reset_force in ["set_read_only"]):
            ...
            for inverter in self.inverters:
                ...
                if self.set_charge_window or (self.inverter_needs_reset_force in ["set_read_only", "mode"]):
                    await inverter.async_adjust_charge_rate(inverter.battery_rate_max_charge * MINUTE_WATT)
                    await inverter.async_disable_charge_window()
                    await inverter.async_adjust_battery_target(100.0, False)
                    await inverter.async_adjust_pause_mode()
                    self.isCharging = False
                if self.set_charge_window or self.set_discharge_window or (self.inverter_needs_reset_force in ["set_read_only", "mode"]):
                    await inverter.async_adjust_reserve(0)
                if self.set_discharge_window or (self.inverter_needs_reset_force in ["set_read_only", "mode"]):
                    await inverter.async_adjust_discharge_rate(inverter.battery_rate_max_discharge * MINUTE_WATT)
                    await inverter.async_adjust_force_discharge(False)
                    self.isDischarging = False
    ...
    async def async_execute_plan(self):
        ...
        if self.inverter_needs_reset:
            await self.async_reset_inverter()
        ...
        for inverter in self.inverters:
            ...
            if self.set_charge_window and self.charge_window_best:
                ...
                if ((minutes_start - self.minutes_now) < (24 * 60)) and (minutes_end > self.minutes_now):
                    ...
                    if self.minutes_now >= minutes_start and self.minutes_now < minutes_end:
                        ...
                        await inverter.async_adjust_charge_rate(int(charge_rate * MINUTE_WATT))
                        ...
                        if not self.set_discharge_during_charge and (inverter.soc_percent >= self.charge_limit_percent_best[0] or not self.set_reserve_enable):
                            await inverter.async_adjust_discharge_rate(0)
                            await inverter.async_adjust_pause_mode(pause_discharge=True)
                            resetDischarge = False
                        ...
                        await inverter.async_adjust_charge_immediate(self.charge_limit_percent_best[0])
                        isCharging = True
                    ...
                    if not disabled_charge_window:
                        ...
                        if (self.minutes_now < minutes_end) and (
                            ...
                            if ((minutes_start - self.minutes_now) > self.set_window_minutes) and (minutes_end - self.minutes_now) >= 24 * 60:
                                ...
                                await inverter.async_adjust_charge_window(charge_start_time, charge_end_time, self.minutes_now)
                        else:
                            self.log(
            ...
            if self.set_discharge_window and self.discharge_window_best:
                ...
                if (self.minutes_now >= minutes_start) and (self.minutes_now < minutes_end) and (self.discharge_limits_best[0] < 100.0):
                    if not self.set_discharge_freeze_only and ((self.soc_kw - PREDICT_STEP * inverter.battery_rate_max_discharge_scaled) >= discharge_soc):
                        ...
                        await inverter.async_adjust_discharge_rate(inverter.battery_rate_max_discharge * MINUTE_WATT)
                        await inverter.async_adjust_force_discharge(True, discharge_start_time, discharge_end_time)
                        resetDischarge = False
                        isDischarging = True

                        if self.set_reserve_enable:
                            await inverter.async_adjust_reserve(self.discharge_limits_best[0])
                            setReserve = True

                        status = "Discharging"
                        status_extra = " target {}%-{}%".format(inverter.soc_percent, self.discharge_limits_best[0])
                        # Immediate discharge mode
                        await inverter.async_adjust_discharge_immediate(self.discharge_limits_best[0])
                    else:
                        await inverter.async_adjust_force_discharge(False)
                        disabled_discharge = True
                        if self.set_discharge_freeze:
                            # In discharge freeze mode we disable charging during discharge slots
                            await inverter.async_adjust_charge_rate(0)
                            await inverter.async_adjust_pause_mode(pause_charge=True)
                    ...
                    if (self.minutes_now < minutes_end) and ((minutes_start - self.minutes_now) <= self.set_window_minutes) and self.discharge_limits_best[0]:
                        await inverter.async_adjust_force_discharge(False, discharge_start_time, discharge_end_time)
                        resetReserve = True
                    else:
                        self.log(
            ...
            if not isCharging and (not isDischarging or disabled_discharge) and self.set_charge_window:
                await inverter.async_adjust_charge_immediate(0)
            ...
            if self.set_soc_enable:
                if (isDischarging and not disabled_discharge) and not self.set_reserve_enable:
                    ...
                    await inverter.async_adjust_battery_target(self.discharge_limits_best[0], False)
                elif (
                    self.charge_limit_best
                    ...
                    if inverter.inv_has_charge_enable_time or isCharging:
                        ...
                        if self.set_charge_freeze and (self.charge_limit_best[0] == self.reserve):
                            if isCharging:
                                ...
                                await inverter.async_adjust_battery_target(inverter.soc_percent, True)
                            else:
                            ...
                            if not self.inverter_hybrid and self.inverter_soc_reset and not isCharging:
                                ...
                                await inverter.async_adjust_battery_target(self.charge_limit_percent_best[0], isCharging)
                    else:
                    ...
                    if not inverter.inv_has_target_soc:
                        # If the inverter doesn't support target soc and soc_enable is on then do that logic here:
                        await inverter.async_mimic_target_soc(0)
                    elif not self.inverter_hybrid and self.inverter_soc_reset:
                        if (
                            self.charge_limit_best
                        ...
                        if not inverter.inv_has_charge_enable_time:
                            await inverter.async_adjust_battery_target(0, False)
            ...
            if self.set_soc_enable and self.set_reserve_enable and not setReserve:
                if self.charge_limit_best and (self.minutes_now < inverter.charge_end_time_minutes) and (self.minutes_now >= inverter.charge_start_time_minutes):
                    ...
                    if (inverter.soc_percent > limit) or (not disabled_charge_window):
                        ...
                        await inverter.async_adjust_reserve(min(limit + 1, 100))
                    resetReserve = False
                else:
        ...
        await self.async_set_charge_discharge_status(isCharging and not disabled_charge_window, isDischarging and not disabled_discharge)
        self.isCharging = isCharging
        self.isDischarging = isDischarging
        return status, status_extra

    async def async_fetch_carbon_intensity(self, entity_id):
        """
        ...
        if entity_id:
            ...
            data_all = await self.get_state(entity_id=entity_id, attribute="forecast")
            if data_all:
                carbon_data = await self.async_minute_data(data_all, self.forecast_days, self.now_utc, "intensity", "from", backwards=False, to_key="to")

        entity_id = self.prefix + ".carbon_now"
        state = await self.get_state(entity_id=entity_id)
        if state is not None:
            try:
                carbon_history = await self.async_minute_data_import_export(self.now_utc, entity_id, required_unit="g/kWh", increment=False, smoothing=False)
            except ValueError:
    ...
    async def async_fetch_octopus_rates(self, entity_id, adjust_key=None):
        """
        ...
        if entity_id:
            ...
            if "_current_rate" in entity_id:
                ...
                data_import = await self.get_state(entity_id=prev_rate_id, attribute="rates")
                if data_import:
                    data_all += data_import
                else:
                    prev_rate_id = entity_id.replace("_current_rate", "_previous_rate")
                    data_import = await self.get_state(entity_id=prev_rate_id, attribute="all_rates")
                    if data_import:
                        data_all += data_import
                    else:
            ...
            data_import = (
                await self.get_state(entity_id=current_rate_id, attribute="rates")
                or await self.get_state(entity_id=current_rate_id, attribute="all_rates")
                or await self.get_state(entity_id=current_rate_id, attribute="raw_today")
            )
            if data_import:
                data_all += data_import
            else:
            ...
            if "_current_rate" in entity_id:
                ...
                data_import = await self.get_state(entity_id=next_rate_id, attribute="rates")
                if data_import:
                    data_all += data_import
                else:
                    next_rate_id = entity_id.replace("_current_rate", "_next_rate")
                    data_import = await self.get_state(entity_id=next_rate_id, attribute="all_rates")
                    if data_import:
                        data_all += data_import
            else:
                # Nordpool tomorrow
                data_import = await self.get_state(entity_id=current_rate_id, attribute="raw_tomorrow")
                if data_import:
                    data_all += data_import
        ...
        if data_all:
            ...
            if rate_key not in data_all[0]:
                rate_key = "value"
            rate_data = await self.async_minute_data(
    ...
    async def async_fetch_sensor_data(self):
        ...
        if self.iboost_enable and "iboost_energy_today" in self.args:
            self.iboost_energy_today, iboost_energy_age = await self.async_minute_data_load(self.now_utc, "iboost_energy_today", 1, required_unit="kWh")
        ...
        self.load_forecast = await self.async_fetch_extra_load_forecast(self.now_utc)

        # Load previous load data
        if await self.async_get_arg("ge_cloud_data", False):
            await self.async_download_ge_data(self.now_utc)
        else:
            # Load data
            if "load_today" in self.args:
                self.load_minutes, self.load_minutes_age = await self.async_minute_data_load(self.now_utc, "load_today", self.max_days_previous, required_unit="kWh")
                ...
                if self.load_forecast:
                    ...
                    await self.async_record_status(message="Error - load_today not set correctly", had_errors=True)
                    raise ValueError

            # Load import today data
            if "import_today" in self.args:
                self.import_today = await self.async_minute_data_import_export(self.now_utc, "import_today", scale=self.import_export_scaling, required_unit="kWh")
            ...
            if "export_today" in self.args:
                self.export_today = await self.async_minute_data_import_export(self.now_utc, "export_today", scale=self.import_export_scaling, required_unit="kWh")
            ...
            if "pv_today" in self.args:
                self.pv_today = await self.async_minute_data_import_export(self.now_utc, "pv_today", required_unit="kWh")
        ...
        self.car_charging_energy = await self.async_load_car_energy(self.now_utc)
        ...
        # futurerate data
        self.future_energy_rates_import, self.future_energy_rates_export = await self.async_futurerate_analysis()

        if "rates_import_octopus_url" in self.args:
            # Fixed URL for rate import
            self.log("Downloading import rates directly from url {}".format(await self.async_get_arg("rates_import_octopus_url", indirect=False)))
            self.rate_import = await self.async_download_octopus_rates(await self.async_get_arg("rates_import_octopus_url", indirect=False))
        elif "metric_octopus_import" in self.args:
            # Octopus import rates
            entity_id = await self.async_get_arg("metric_octopus_import", None, indirect=False)
            self.rate_import = await self.async_fetch_octopus_rates(entity_id, adjust_key="is_intelligent_adjusted")
            if not self.rate_import:
                self.log("Error: metric_octopus_import is not set correctly or no energy rates can be read")
                await self.async_record_status(message="Error - metric_octopus_import not set correctly or no energy rates can be read", had_errors=True)
                raise ValueError
        else:
            # Basic rates defined by user over time
            self.rate_import = await self.async_basic_rates(await self.async_get_arg("rates_import", [], indirect=False), "import")

        # Gas rates if set
        if "metric_octopus_gas" in self.args:
            entity_id = await self.async_get_arg("metric_octopus_gas", None, indirect=False)
            self.rate_gas = await self.async_fetch_octopus_rates(entity_id)
            if not self.rate_gas:
                self.log("Error: metric_octopus_gas is not set correctly or no energy rates can be read")
                await self.async_record_status(message="Error - rate_import_gas not set correctly or no energy rates can be read", had_errors=True)
                raise ValueError
            self.rate_gas, self.rate_gas_replicated = await self.async_rate_replicate(self.rate_gas, is_import=False, is_gas=False)
            self.rate_gas = await self.async_rate_scan_gas(self.rate_gas, print=True)
        elif "rates_gas" in self.args:
            self.rate_gas = await self.async_basic_rates(await self.async_get_arg("rates_gas", [], indirect=False), "gas")
            self.rate_gas, self.rate_gas_replicated = await self.async_rate_replicate(self.rate_gas, is_import=False, is_gas=False)
            self.rate_gas = await self.async_rate_scan_gas(self.rate_gas, print=True)

        # Carbon intensity data
        if self.carbon_enable and ("carbon_intensity" in self.args):
            entity_id = await self.async_get_arg("carbon_intensity", None, indirect=False)
            self.carbon_intensity, self.carbon_history = await self.async_fetch_carbon_intensity(entity_id)

        # Work out current car SOC and limit
        self.car_charging_loss = 1 - float(await self.async_get_arg("car_charging_loss"))
        ...
        if "octopus_intelligent_slot" in self.args:
            ...
            try:
                ...
                await self.async_record_status(message="Error - octopus_intelligent_slot not set correctly", had_errors=True)
            ...
            if self.rate_import:
                self.rate_import = await self.async_rate_scan(self.rate_import, print=False)
            ...
            if self.num_cars >= 1:
                ...
                if vehicle:
                    ...
                    size = await self.get_state(entity_id=entity_id, attribute="vehicle_battery_size_in_kwh")
                    rate = await self.get_state(entity_id=entity_id, attribute="charge_point_power_in_kw")
                    if size:
                ...
                self.car_charging_limit[0] = self.dp3((float(await self.async_get_arg("car_charging_limit", 100.0, index=0)) * self.car_charging_battery_size[0]) / 100.0)
                ...
                if vehicle_pref and self.octopus_intelligent_charging:
                    ...
                    octopus_ready_time = await self.async_get_arg("octopus_ready_time", None)
                    octopus_limit = await self.async_get_arg("octopus_charge_limit", None)
                    if octopus_limit:
                        try:
                ...
                if self.octopus_intelligent_charging:
                    self.octopus_slots = await self.async_add_now_to_octopus_slot(self.octopus_slots, self.now_utc)
                    if not self.octopus_intelligent_ignore_unplugged or self.car_charging_planned[0]:
                        self.car_charging_slots[0] = await self.async_load_octopus_slots(self.octopus_slots)
                        self.log(
        ...
        for car_n in range(self.num_cars):
            if (car_n == 0) and self.car_charging_manual_soc:
                self.car_charging_soc[car_n] = await self.async_get_arg("car_charging_manual_soc_kwh")
            else:
                self.car_charging_soc[car_n] = (await self.async_get_arg("car_charging_soc", 0.0, index=car_n) * self.car_charging_battery_size[car_n]) / 100.0
        if self.num_cars:
            self.log(
        ...
        if "rates_export_octopus_url" in self.args:
            ...
            self.log("Downloading export rates directly from url {}".format(await self.async_get_arg("rates_export_octopus_url", indirect=False)))
            self.rate_export = await self.async_download_octopus_rates(await self.async_get_arg("rates_export_octopus_url", indirect=False))
        elif "metric_octopus_export" in self.args:
            # Octopus export rates
            entity_id = await self.async_get_arg("metric_octopus_export", None, indirect=False)
            self.rate_export = await self.async_fetch_octopus_rates(entity_id)
            if not self.rate_export:
                self.log("Warning: metric_octopus_export is not set correctly or no energy rates can be read")
                await self.async_record_status(message="Error - metric_octopus_export not set correctly or no energy rates can be read", had_errors=True)
        else:
            # Basic rates defined by user over time
            self.rate_export = await self.async_basic_rates(await self.async_get_arg("rates_export", [], indirect=False), "export")
        ...
        if "octopus_saving_session" in self.args:
            ...
            octopoints_per_penny = await self.async_get_arg("octopus_saving_session_octopoints_per_penny", 8)  # Default 8 octopoints per found

            entity_id = await self.async_get_arg("octopus_saving_session", indirect=False)
            if entity_id:
                state = await self.async_get_arg("octopus_saving_session", False)

                joined_events = await self.get_state(entity_id=entity_id, attribute="joined_events")
                if not joined_events:
                    entity_id = entity_id.replace("binary_sensor.", "event.").replace("_sessions", "_session_events")
                    joined_events = await self.get_state(entity_id=entity_id, attribute="joined_events")

                available_events = await self.get_state(entity_id=entity_id, attribute="available_events")
                if available_events:
                    for event in available_events:
                        code = event.get("code", None)  # decode the available events structure for code, start/end time & rate
                        start = event.get("start", None)
                        end = event.get("end", None)
                        start_time = await self.async_str2time(start)  # reformat the saving session start & end time for improved readability
                        end_time = await self.async_str2time(end)
                        ...
                        if code:  # Join the new Octopus saving event and send an alert
                            ...
                            await self.call_service("octopus_energy/join_octoplus_saving_session_event", event_code=code, entity_id=entity_id)
                            await self.async_call_notify(
                ...
                if joined_events:
                    for event in joined_events:
                        ...
                        if start and end and saving_rate > 0:
                            ...
                            try:
                                start_time = await self.async_str2time(start)
                                end_time = await self.async_str2time(end)
        ...
        # Standing charge
        self.metric_standing_charge = await self.async_get_arg("metric_standing_charge", 0.0) * 100.0
        self.log("Standing charge is set to {} p".format(self.metric_standing_charge))

        # Replicate and scan import rates
        if self.rate_import:
            self.rate_import = await self.async_rate_scan(self.rate_import, print=False)
            self.rate_import, self.rate_import_replicated = await self.async_rate_replicate(self.rate_import, self.io_adjusted, is_import=True)
            ...
            await self.async_record_status(message="Error - No import rate data provided", had_errors=True)

        # Replicate and scan export rates
        if self.rate_export:
            self.rate_export = await self.async_rate_scan_export(self.rate_export, print=False)
            self.rate_export, self.rate_export_replicated = await self.async_rate_replicate(self.rate_export, is_import=False)
            ...
            await self.async_record_status(message="Error - No export rate data provided", had_errors=True)

        # Set rate thresholds
        if self.rate_import or self.rate_export:
            await self.async_set_rate_thresholds()

        # Find discharging windows
        if self.rate_export:
            self.high_export_rates, lowest, highest = await self.async_rate_scan_window(self.rate_export, 5, self.rate_export_cost_threshold, True)
        ...
        if self.rate_import:
            ...
            self.low_rates, lowest, highest = await self.async_rate_scan_window(self.rate_import, 5, self.rate_import_cost_threshold, False)
        ...
        for car_n in range(self.num_cars):
            if self.octopus_intelligent_charging and car_n == 0:
                ...
                self.car_charging_slots[car_n] = await self.async_plan_car_charging(car_n, self.low_rates)
            else:
        ...
        await self.async_publish_car_plan()

        # Work out cost today
        if self.import_today:
            self.cost_today_sofar, self.carbon_today_sofar = await self.async_today_cost(self.import_today, self.export_today)

        # Fetch PV forecast if enabled, today must be enabled, other days are optional
        self.pv_forecast_minute, self.pv_forecast_minute10 = await self.async_fetch_pv_forecast()

        # Apply modal filter to historical data
        if self.load_minutes and not self.load_forecast_only:
            await self.async_previous_days_modal_filter(self.load_minutes)
    ...
    async def async_publish_rate_and_threshold(self):
        """
        ...
        if self.rate_export:
            ...
            await self.async_publish_rates(self.rate_export, True)
        ...
        if self.rate_import:
            ...
            await self.async_publish_rates(self.rate_import, False)

        # And gas
        if self.rate_gas:
            await self.async_publish_rates(self.rate_gas, False, gas=True)

    async def async_fetch_inverter_data(self):
        """
        Fetch data about the inverters
        """
        # Find the inverters
        self.num_inverters = int(await self.async_get_arg("num_inverters", 1))
        ...
        for id in range(self.num_inverters):
            ...
            await inverter.async_init_inverter()
            await inverter.async_update_status(self.minutes_now)
            ...
            if id == 0 and (not self.computed_charge_curve or self.battery_charge_power_curve_auto) and not self.battery_charge_power_curve:
                curve = await inverter.async_find_charge_curve(discharge=False)
            ...
            if id == 0 and (not self.computed_discharge_curve or self.battery_discharge_power_curve_auto) and not self.battery_discharge_power_curve:
                curve = await inverter.async_find_charge_curve(discharge=True)
        ...
        await self.async_publish_charge_limit(self.charge_limit, self.charge_window, self.charge_limit_percent, best=False)
    ...
    async def async_fetch_config_options(self):
        """
        Fetch all the configuration options
        """

        self.debug_enable = await self.async_get_arg("debug_enable")
        self.previous_status = await self.get_state(self.prefix + ".status")
        forecast_hours = await self.async_get_arg("forecast_hours", 48)
        self.calculate_fast_plan = await self.async_get_arg("calculate_fast_plan")
        ...
        self.num_cars = await self.async_get_arg("num_cars", 1)
        self.calculate_plan_every = max(await self.async_get_arg("calculate_plan_every"), 5)
        ...
        # Days previous
        self.holiday_days_left = await self.async_get_arg("holiday_days_left")
        self.load_forecast_only = await self.async_get_arg("load_forecast_only", False)

        self.days_previous = await self.async_get_arg("days_previous", [7])
        self.days_previous_weight = await self.async_get_arg("days_previous_weight", [1 for i in range(len(self.days_previous))])
        ...
        self.forecast_plan_hours = max(min(await self.async_get_arg("forecast_plan_hours", 96), forecast_hours), 8)
        self.inverter_clock_skew_start = await self.async_get_arg("inverter_clock_skew_start", 0)
        self.inverter_clock_skew_end = await self.async_get_arg("inverter_clock_skew_end", 0)
        self.inverter_clock_skew_discharge_start = await self.async_get_arg("inverter_clock_skew_discharge_start", 0)
        self.inverter_clock_skew_discharge_end = await self.async_get_arg("inverter_clock_skew_discharge_end", 0)
        ...
        # Metric config
        self.metric_min_improvement = await self.async_get_arg("metric_min_improvement")
        self.metric_min_improvement_discharge = await self.async_get_arg("metric_min_improvement_discharge")
        self.metric_battery_cycle = await self.async_get_arg("metric_battery_cycle")
        self.metric_future_rate_offset_import = await self.async_get_arg("metric_future_rate_offset_import")
        self.metric_future_rate_offset_export = await self.async_get_arg("metric_future_rate_offset_export")
        self.metric_inday_adjust_damping = await self.async_get_arg("metric_inday_adjust_damping")
        self.rate_low_threshold = await self.async_get_arg("rate_low_threshold")
        self.rate_high_threshold = await self.async_get_arg("rate_high_threshold")
        self.inverter_soc_reset = await self.async_get_arg("inverter_soc_reset")

        self.metric_battery_value_scaling = await self.async_get_arg("metric_battery_value_scaling")
        self.notify_devices = await self.async_get_arg("notify_devices", ["notify"])
        self.pv_scaling = await self.async_get_arg("pv_scaling")
        self.pv_metric10_weight = await self.async_get_arg("pv_metric10_weight")
        self.load_scaling = await self.async_get_arg("load_scaling")
        self.load_scaling10 = await self.async_get_arg("load_scaling10")
        self.load_scaling_saving = await self.async_get_arg("load_scaling_saving")
        self.battery_rate_max_scaling = await self.async_get_arg("battery_rate_max_scaling")
        self.battery_rate_max_scaling_discharge = await self.async_get_arg("battery_rate_max_scaling_discharge")

        self.best_soc_step = 0.25
        self.metric_cloud_enable = await self.async_get_arg("metric_cloud_enable")
        self.metric_load_divergence_enable = await self.async_get_arg("metric_load_divergence_enable")

        # Battery charging options
        self.battery_capacity_nominal = await self.async_get_arg("battery_capacity_nominal")
        self.battery_loss = 1.0 - await self.async_get_arg("battery_loss")
        self.battery_loss_discharge = 1.0 - await self.async_get_arg("battery_loss_discharge")
        self.inverter_loss = 1.0 - await self.async_get_arg("inverter_loss")
        self.inverter_hybrid = await self.async_get_arg("inverter_hybrid")
        ...
        if self.args.get("battery_charge_power_curve", "") == "auto":
            ...
            if not isinstance(self.battery_charge_power_curve, dict):
                ...
                await self.async_record_status("battery_charge_power_curve is incorrectly configured - ignoring", had_errors=True)
        ...
        if self.args.get("battery_discharge_power_curve", "") == "auto":
            ...
            if not isinstance(self.battery_discharge_power_curve, dict):
                ...
                await self.async_record_status("battery_discharge_power_curve is incorrectly configured - ignoring", had_errors=True)

        self.import_export_scaling = await self.async_get_arg("import_export_scaling", 1.0)
        self.best_soc_margin = 0.0
        self.best_soc_min = await self.async_get_arg("best_soc_min")
        self.best_soc_max = await self.async_get_arg("best_soc_max")
        self.best_soc_keep = await self.async_get_arg("best_soc_keep")
        self.set_soc_minutes = 30
        self.set_window_minutes = 30
        self.octopus_intelligent_charging = await self.async_get_arg("octopus_intelligent_charging")
        self.octopus_intelligent_ignore_unplugged = await self.async_get_arg("octopus_intelligent_ignore_unplugged")
        await self.async_get_car_charging_planned()
        self.load_inday_adjustment = 1.0

        self.combine_rate_threshold = await self.async_get_arg("combine_rate_threshold")
        self.combine_discharge_slots = await self.async_get_arg("combine_discharge_slots")
        self.combine_charge_slots = await self.async_get_arg("combine_charge_slots")
        self.charge_slot_split = 60
        self.discharge_slot_split = 60 if self.calculate_fast_plan else 30
        self.calculate_best = True
        self.set_read_only = await self.async_get_arg("set_read_only")

        # hard wired options, can be configured per inverter later on
        self.set_soc_enable = True
        self.set_reserve_enable = await self.async_get_arg("set_reserve_enable")
        self.set_reserve_hold = True
        self.set_discharge_freeze = True
        self.set_charge_freeze = await self.async_get_arg("set_charge_freeze")
        self.set_charge_low_power = await self.async_get_arg("set_charge_low_power")
        self.calculate_discharge_first = True

        self.set_status_notify = await self.async_get_arg("set_status_notify")
        self.set_inverter_notify = await self.async_get_arg("set_inverter_notify")
        self.set_discharge_freeze_only = await self.async_get_arg("set_discharge_freeze_only")
        self.set_discharge_during_charge = await self.async_get_arg("set_discharge_during_charge")

        # Mode
        self.predbat_mode = await self.async_get_arg("mode")
        if self.predbat_mode == PREDBAT_MODE_OPTIONS[PREDBAT_MODE_CONTROL_SOC]:
            ...
            await self.async_expose_config("mode", self.predbat_mode)

        self.log("Predbat mode is set to {}".format(self.predbat_mode))

        self.calculate_discharge_oncharge = await self.async_get_arg("calculate_discharge_oncharge")
        self.calculate_second_pass = await self.async_get_arg("calculate_second_pass")
        self.calculate_inday_adjustment = await self.async_get_arg("calculate_inday_adjustment")
        self.calculate_tweak_plan = await self.async_get_arg("calculate_tweak_plan")
        self.calculate_regions = True
        self.calculate_secondary_order = await self.async_get_arg("calculate_secondary_order")

        self.balance_inverters_enable = await self.async_get_arg("balance_inverters_enable")
        self.balance_inverters_charge = await self.async_get_arg("balance_inverters_charge")
        self.balance_inverters_discharge = await self.async_get_arg("balance_inverters_discharge")
        self.balance_inverters_crosscharge = await self.async_get_arg("balance_inverters_crosscharge")
        self.balance_inverters_threshold_charge = max(await self.async_get_arg("balance_inverters_threshold_charge"), 1.0)
        self.balance_inverters_threshold_discharge = max(await self.async_get_arg("balance_inverters_threshold_discharge"), 1.0)

        if self.set_read_only:
            self.log("NOTE: Read-only mode is enabled, the inverter controls will not be used!!")

        # Enable load filtering
        self.load_filter_modal = await self.async_get_arg("load_filter_modal")

        # iBoost solar diverter model
        self.iboost_enable = await self.async_get_arg("iboost_enable")
        self.carbon_enable = await self.async_get_arg("carbon_enable")
        self.carbon_metric = await self.async_get_arg("carbon_metric")
        self.iboost_solar = await self.async_get_arg("iboost_solar")
        self.iboost_gas = await self.async_get_arg("iboost_gas")
        self.iboost_charging = await self.async_get_arg("iboost_charging")
        self.iboost_gas_scale = await self.async_get_arg("iboost_gas_scale")

        self.iboost_max_energy = await self.async_get_arg("iboost_max_energy")
        self.iboost_max_power = await self.async_get_arg("iboost_max_power") / MINUTE_WATT
        self.iboost_min_power = await self.async_get_arg("iboost_min_power") / MINUTE_WATT
        self.iboost_min_soc = await self.async_get_arg("iboost_min_soc")
        self.iboost_today = await self.async_get_arg("iboost_today")
        self.iboost_next = self.iboost_today
        self.iboost_running = False
        self.iboost_energy_today = {}

        # Car options
        self.car_charging_hold = await self.async_get_arg("car_charging_hold")
        self.car_charging_manual_soc = await self.async_get_arg("car_charging_manual_soc")
        self.car_charging_threshold = float(await self.async_get_arg("car_charging_threshold")) / 60.0
        self.car_charging_energy_scale = await self.async_get_arg("car_charging_energy_scale")

        # Update list of slot times
        self.manual_charge_times = await self.async_manual_times("manual_charge")
        self.manual_discharge_times = await self.async_manual_times("manual_discharge")
        self.manual_freeze_charge_times = await self.async_manual_times("manual_freeze_charge")
        self.manual_freeze_discharge_times = await self.async_manual_times("manual_freeze_discharge")
        self.manual_idle_times = await self.async_manual_times("manual_idle")
        self.manual_all_times = (
    ...
    async def async_update_time(self, print=True):
        """
        Update the current time/date
        """
        local_tz = pytz.timezone(await self.async_get_arg("timezone", "Europe/London"))
        skew = await self.async_get_arg("clock_skew", 0)
        if skew:
    ...
    @ad.app_lock
    async def async_update_pred(self, scheduled=True):
        """
        ...
        await self.async_expose_config("active", True)

        # Check our version
        await self.async_download_predbat_releases()

        await self.async_fetch_config_options()
        await self.async_fetch_sensor_data()
        await self.async_fetch_inverter_data()
        ...
        recompute = await self.async_calculate_plan(recompute=recompute)

        # Publish rate data
        await self.async_publish_rate_and_threshold()

        # Execute the plan, re-read the inverter first if we had to calculate (as time passes during calculations)
        if recompute:
            await self.async_fetch_inverter_data()
        status, status_extra = await self.async_execute_plan()
        ...
        if not recompute:
            ...
            if (plan_age_minutes + RUN_EVERY) > self.calculate_plan_every:
                ...
                await self.async_calculate_plan(recompute=True)
                await self.async_fetch_inverter_data()
                status, status_extra = await self.async_execute_plan()
            else:
        ...
        if self.iboost_enable and scheduled:
            ...
            await self.async_expose_config("iboost_today", self.iboost_next)
            ...
            for car_n in range(self.num_cars):
                if (car_n == 0) and self.car_charging_manual_soc:
                    ...
                    if self.car_charging_soc_next[car_n] is not None:
                        await self.async_expose_config("car_charging_manual_soc_kwh", round(self.car_charging_soc_next[car_n], 2))
        ...
        await self.async_expose_config("active", False)
    ...
    async def async_download_predbat_file_from_github(self, tag, filename, new_filename):
        """
    ...
    async def async_download_predbat_version(self, version):
        ...
        if tag_split:
            ...
            # Download predbat.py
            file = "predbat.py"
            predbat_code = await self.async_download_predbat_file_from_github(tag, file, os.path.join(this_path, file + "." + tag))
            if predbat_code:
                ...
                if files:
                    for file in files:
                        # Download the remaining files
                        if file != "predbat.py":
                            await self.async_download_predbat_file_from_github(tag, file, os.path.join(this_path, file + "." + tag))
    ...
    async def async_expose_config(self, name, value, quiet=True, event=False, force=False, in_progress=False):
        ...
        if item:
            enabled = await self.async_user_config_item_enabled(item)
            if not enabled:
                ...
                if entity and ((item.get("value") is None) or (value != item["value"]) or force):
                    ...
                    if item["type"] == "input_number":
                        ...
                        await self.set_state(entity_id=entity, state=("on" if value else "off"), attributes={"friendly_name": item["friendly_name"], "icon": icon})
                        ...
                        if old_state and old_state != value:
                            await self.set_state(entity_id=entity, state=old_state, attributes={"friendly_name": item["friendly_name"], "options": options, "icon": icon})
                        await self.set_state(entity_id=entity, state=value, attributes={"friendly_name": item["friendly_name"], "options": options, "icon": icon})
    ...
    async def async_user_config_item_enabled(self, item):
        """
    ...
    async def async_dashboard_item(self, entity, state, attributes):
        """
        Publish state and log dashboard item
        """
        await self.set_state(entity_id=entity, state=state, attributes=attributes)
    ...
    async def async_create_debug_yaml(self):
        """
    ...
    async def async_create_entity_list(self):
        """
        ...
        for try_enable in enable_list:
            for item in CONFIG_ITEMS:
                ...
                if entity and enable == try_enable and await self.async_user_config_item_enabled(item):
                    text += "  - entity: " + entity + "\n"
    ...
    async def async_load_previous_value_from_ha(self, entity):
        """
        Load HA value either from state or from history if there is any
        """
        ha_value = await self.get_state(entity)
        if ha_value is not None:
            return ha_value
        history = await self.async_get_history_async(entity_id=entity)
        if history:
            history = history[0]
            ha_value = history[-1]["state"]
        return ha_value

    async def async_load_user_config(self, quiet=True, register=False):
        """
        Load config from HA
        """
        self.config_index = {}
        ...
        new_install = True
        current_status = await self.async_load_previous_value_from_ha(self.prefix + ".status")
        if current_status:
            new_install = False
        ...
        for item in CONFIG_ITEMS:
            ...
            if not enabled:
                ...
                ha_value = await self.get_state(entity)
                if ha_value is not None:
                    await self.set_state(entity_id=entity, state=ha_value, attributes={"friendly_name": "[Disabled] " + item["friendly_name"]})
                continue

            # Get from current state?
            ha_value = await self.async_load_previous_value_from_ha(entity)
            ...
            if ha_value is not None:
                if item.get("manual"):
                    await self.async_manual_times(name, new_value=ha_value)
                else:
                    await self.async_expose_config(item["name"], ha_value, quiet=quiet)
        ...
        if register:
            ...
            watch_list = await self.async_get_arg("watch_list", [], indirect=False)
    ...
    async def async_resolve_arg_re(self, arg, arg_value, state_keys):
        """
        ...
        if isinstance(arg_value, list):
            ...
            for item_value in arg_value:
                item_matched, item_value = await self.async_resolve_arg_re(arg, item_value, state_keys)
                if not item_matched:
    ...
    async def async_auto_config(self):
        """
        Auto configure
        ...
        for arg in self.args:
            ...
            matched, arg_value = await self.async_resolve_arg_re(arg, arg_value, state_keys)
            if not matched:
    ...
    async def async_sanity(self):
        ...
        if passed:
            ...
            await self.async_record_status("WARN: Sanity startup checked has FAILED, see your logs for details")
        return passed
    ...
    async def initialize(self):
        ...
        try:
            ...
            await self.async_sanity()
            await self.async_auto_config()
            await self.async_load_user_config(quiet=False, register=True)
        except Exception as e:
            self.log("ERROR: Exception raised {}".format(e))
            await self.async_record_status("ERROR: Exception raised {}".format(e))
            raise e

        # Catch template configurations and exit
        if await self.async_get_arg("template", False):
            ...
            await self.async_record_status("ERROR: You still have a template configuration, please edit apps.yaml or restart AppDaemon if you just updated with HACS")
            return

        if SIMULATE and SIMULATE_LENGTH:
            # run once to get data
            SIMULATE = False
            await self.async_update_pred(scheduled=False)
            ...
            for offset in range(0, SIMULATE_LENGTH, 30):
                ...
                await self.async_update_pred(scheduled=True)
        else:
            ...
            if not INVERTER_TEST:
                ...
                await self.async_update_time_loop(None)

            # Balance inverters
            run_every_balance = await self.async_get_arg("balance_inverters_seconds", 60)
    ...
    def run_time_loop(self, cb_args):
        if self.run_time_task:
            if not self.run_time_task.done():
                self.log("Run time loop already running, skipping")
                return
        self.run_time_task = self.create_task(self.async_run_time_loop())

    def update_time_loop(self, cb_args):
        if self.update_time_task:
            if not self.update_time_task.done():
    ...
    def run_time_loop_balance(self, cb_args):
        if self.balance_time_task:
            if not self.balance_time_task.done():
                self.log("Balance time loop already running, skipping")
                return
        self.balance_time_task = self.create_task(self.async_run_time_loop_balance())

    @ad.app_lock
    async def async_update_time_loop(self):
        """
        Called every 15 seconds
        """
        if self.update_pending and not self.prediction_started:
            self.prediction_started = True
            await self.async_load_user_config()
            self.update_pending = False
            try:
                await self.async_update_pred(scheduled=False)
                await self.async_create_entity_list()
            except Exception as e:
                self.log("ERROR: Exception raised {}".format(e))
                await self.async_record_status("ERROR: Exception raised {}".format(e))
                raise e
    ...
    @ad.app_lock
    async def async_run_time_loop(self):
        """
    ...
    async def async_run_time_loop_balance(self):
        """
        ...
        if not self.prediction_started and self.balance_inverters_enable and not self.set_read_only:
            try:
                await self.async_balance_inverters()
            except Exception as e:
                self.log("ERROR: Exception raised {}".format(e))
                await self.async_record_status("ERROR: Exception raised {}".format(e))
                raise
