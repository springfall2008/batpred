...
def reset_inverter(my_predbat):
    ...
    my_predbat.battery_rate_max_charge_scaled = 10 / 60.0
    my_predbat.battery_rate_max_discharge_scaled = 10 / 60.0
    my_predbat.battery_rate_min = 0
    my_predbat.charge_rate_now = 10 / 60.0
    my_predbat.discharge_rate_now = 10 / 60.0
...
def simple_scenario(name, my_predbat, load_amount, pv_amount, assert_final_metric, assert_final_soc, with_battery=True, battery_loss=1.0, battery_size=100.0, battery_soc=0.0):
    """
    No PV, No Load
    """
...
def main():
    ...
    failed |= simple_scenario("pv_only_bat", my_predbat, 0, 1, assert_final_metric=-export_rate * 14, assert_final_soc=10, with_battery=True, battery_size=10)
    if failed:
