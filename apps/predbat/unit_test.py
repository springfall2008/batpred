...
def run_execute_tests(my_predbat):    
    ...
    failed |= run_execute_test(my_predbat, "nocharge2", set_charge_window=True, set_discharge_window=True)
    failed |= run_execute_test(my_predbat, "nochargefuture", set_charge_window=True, set_discharge_window=True, charge_window_best=charge_window_best4, charge_limit_best=charge_limit_best)
    ...
    failed |= run_execute_test(my_predbat, "chargeshift", charge_window_best=charge_window_best3, charge_limit_best=charge_limit_best, assert_charge_time_enable=True, set_charge_window=True, assert_status="Charging", assert_charge_start_time_minutes=-1, assert_charge_end_time_minutes=my_predbat.minutes_now + 60) 
    failed |= run_execute_test(my_predbat, "chargeshift2", charge_window_best=charge_window_best5, charge_limit_best=charge_limit_best, assert_charge_time_enable=True, set_charge_window=True, assert_status="Charging", assert_charge_start_time_minutes=-1, assert_charge_end_time_minutes=my_predbat.minutes_now + 60) 
    failed |= run_execute_test(my_predbat, "chargeshift3", charge_window_best=charge_window_best5, charge_limit_best=charge_limit_best, assert_charge_time_enable=True, set_charge_window=True, assert_status="Charging", inverter_charge_time_minutes_start=-24*60, assert_charge_start_time_minutes=my_predbat.minutes_now, assert_charge_end_time_minutes=my_predbat.minutes_now + 60) 
    failed |= run_execute_test(my_predbat, "calib", in_calibration=True, assert_status="Calibration", assert_charge_time_enable=True, assert_charge_end_time_minutes=my_predbat.minutes_now + 60)
    failed |= run_execute_test(my_predbat, "nocharge3", set_charge_window=True, set_discharge_window=True)
    failed |= run_execute_test(my_predbat, "chargero", charge_window_best=charge_window_best, charge_limit_best=charge_limit_best, set_charge_window=True, read_only=True, assert_status="Read-Only")
    ...
    failed |= run_execute_test(my_predbat, "chargehold", charge_window_best=charge_window_best, charge_limit_best=charge_limit_best, assert_charge_time_enable=False, set_charge_window=True, soc_kw=10, assert_pause_discharge=True, assert_status="Hold charging", assert_discharge_rate=0, assert_reserve=100)
    failed |= run_execute_test(my_predbat, "chargehold2", charge_window_best=charge_window_best, charge_limit_best=charge_limit_best2, assert_charge_time_enable=False, set_charge_window=True, soc_kw=5, assert_pause_discharge=True, assert_status="Hold charging", assert_discharge_rate=0, assert_reserve=51)
    failed |= run_execute_test(my_predbat, "chargefreeze", charge_window_best=charge_window_best, charge_limit_best=charge_limit_best_frz, assert_charge_time_enable=False, set_charge_window=True, soc_kw=10, assert_pause_discharge=True, assert_status="Freeze charging", assert_discharge_rate=0, assert_reserve=100)
    failed |= run_execute_test(my_predbat, "chargefreeze2", charge_window_best=charge_window_best, charge_limit_best=charge_limit_best_frz, assert_charge_time_enable=False, set_charge_window=True, soc_kw=5, assert_pause_discharge=True, assert_status="Freeze charging", assert_discharge_rate=0, assert_reserve=51)
    failed |= run_execute_test(my_predbat, "chargefreeze3", charge_window_best=charge_window_best2, charge_limit_best=charge_limit_best_frz, assert_charge_time_enable=False, set_charge_window=True, soc_kw=5)
    failed |= run_execute_test(my_predbat, "nocharge4", set_charge_window=True, set_discharge_window=True)
    failed |= run_execute_test(my_predbat, "chargelater", charge_window_best=charge_window_best2, charge_limit_best=charge_limit_best, assert_charge_time_enable=True, set_charge_window=True, assert_status="Idle", assert_charge_start_time_minutes=my_predbat.minutes_now + 30, assert_charge_end_time_minutes=my_predbat.minutes_now + 60)
    failed |= run_execute_test(my_predbat, "chargelater2", charge_window_best=charge_window_best6, charge_limit_best=charge_limit_best, assert_charge_time_enable=False, set_charge_window=True, assert_status="Idle")
    failed |= run_execute_test(my_predbat, "nocharge5", set_charge_window=True, set_discharge_window=True)
    ...
    failed |= run_execute_test(my_predbat, "dischargefreeze", discharge_window_best=discharge_window_best, discharge_limits_best=discharge_limits_best_frz, assert_force_discharge=False, set_discharge_window=True, soc_kw=9, assert_status="Freeze discharging", assert_pause_charge=True, assert_charge_rate=0)
    failed |= run_execute_test(my_predbat, "dischargefreeze2", discharge_window_best=discharge_window_best2, discharge_limits_best=discharge_limits_best_frz, assert_force_discharge=False, set_discharge_window=True, soc_kw=9, assert_status="Freeze discharging", assert_pause_charge=True, assert_charge_rate=0)
    failed |= run_execute_test(my_predbat, "nocharge5", set_charge_window=True, set_discharge_window=True)
    return failed
