    ...
    def initialize(self, only=None, phase=0):
        ...
        for component_name, component_info in COMPONENT_LIST.items():
            ...
            if have_all_args:
                ...
                # setting up. Incidental shared settings must not count - gecloud_data lists
                # the general-purpose days_previous, which every default config sets, so
                # counting all args would warn every user about an integration they never
                # configured.
