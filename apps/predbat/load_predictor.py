...
MODEL_VERSION = 3  # Bumped for larger network
...
TOTAL_FEATURES = NUM_LOAD_FEATURES + NUM_TIME_FEATURES
...
class LoadPredictor:
    ...
    def _load_to_energy_per_step(self, load_minutes, step=STEP_MINUTES):
        ...
        The load_minutes dict contains cumulative kWh values going backwards in time,
        where minute 0 is now and higher minutes are further in the past.
    ...
    def _create_dataset(self, load_minutes, now_utc, is_finetune=False, time_decay_days=7, validation_holdout_hours=24):
        """
        ...
        Training uses days 2-7 of data, with the most recent 24h held out for validation.
        This allows validating the model's ability to predict "tomorrow" from "today's" data.
        ...
        # Split point: validation uses most recent data (minute 0 to validation_holdout)
        # Training uses older data (validation_holdout to end_minute)
        ...
        # Create training samples (from older data, after validation holdout)
        # These samples predict targets in the range [validation_end, end_minute - lookback]
        for target_minute in range(validation_end, end_minute - LOOKBACK_STEPS * STEP_MINUTES, STEP_MINUTES):
            ...
            # Combine features: [lookback..., time_features...]
            features = np.concatenate([np.array(lookback_values, dtype=np.float32), time_features])
    ...
    def train(self, load_minutes, now_utc, is_initial=True, epochs=50, time_decay_days=7, patience=5):
        """
        Train or fine-tune the model.
        
        Training uses days 2-7 of data, with the most recent 24 hours held out
        for validation. This tests the model's ability to predict "tomorrow"
        given "today's" patterns.
        ...
        """
        self.log("ML Predictor: Starting {} training with {} epochs".format(
            "initial" if is_initial else "fine-tune", epochs))
        
        # Create dataset with train/validation split
        result = self._create_dataset(
            load_minutes, now_utc, 
            is_finetune=not is_initial, 
        ...
        self.log("ML Predictor: Created {} training samples, {} validation samples".format(
        ...
        for epoch in range(epochs):
            ...
            self.log("ML Predictor: Epoch {}/{}: train_loss={:.4f} val_mae={:.4f} kWh".format(
                epoch + 1, epochs, epoch_loss, val_mae))
    ...
    def predict(self, load_minutes, now_utc, midnight_utc, exog_features=None):
        """
        ...
        for step_idx in range(PREDICT_HORIZON):
            ...
            # Combine features: lookback + time features for target
    ...
    def load(self, filepath):
        ...
        try:
            ...
            if saved_version != MODEL_VERSION:
                self.log("Warn: ML Predictor: Model version mismatch (saved={}, current={}), retraining from scratch".format(
                    saved_version, MODEL_VERSION))
                return False
            
            # Check architecture compatibility
            if metadata.get("lookback_steps") != LOOKBACK_STEPS or \
               metadata.get("output_steps") != OUTPUT_STEPS or \
               metadata.get("hidden_sizes") != HIDDEN_SIZES:
            ...
            self.log("ML Predictor: Model loaded from {} (trained {}, val_mae={:.4f})".format(
                filepath,
                self.training_timestamp.strftime("%Y-%m-%d %H:%M") if self.training_timestamp else "unknown",
                self.validation_mae if self.validation_mae else 0
            ))
            return True
