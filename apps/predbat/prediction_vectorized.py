...
class PredictionVectorized:
    ...
    def run_prediction_batch(self, charge_window_enable, export_window_enable):
        ...
        for step_idx in range(self.num_steps):
            ...
            battery_draw = np.where(
                force_discharge,
                np.minimum(discharge_rate_step, battery_to_min),
                battery_draw
            )
            ...
            battery_draw = np.where(
                force_charge,
                -np.minimum(charge_rate_step, battery_to_max),
                battery_draw
            )
            ...
            battery_draw = np.where(
                battery_draw > 0,
                np.minimum(battery_draw, inverter_limit_step),
                battery_draw
            )
            # Limit charge
            battery_draw = np.where(
                battery_draw < 0,
                np.maximum(battery_draw, -inverter_limit_step),
                battery_draw
            )
            ...
            soc_delta = np.where(
                battery_draw > 0,
                -battery_draw / self.battery_loss_discharge,  # Discharge
                -battery_draw * self.battery_loss  # Charge (battery_draw is negative)
            )
...
if __name__ == "__main__":
    ...
    class DummyBase:
        def __init__(self):
            ...
            self.battery_charge_power_curve = [
                [0, 1.0], [50, 1.0], [90, 0.8], [100, 0.3]
            ]
            self.battery_discharge_power_curve = [
                [0, 0.3], [10, 0.8], [50, 1.0], [100, 1.0]
            ]
    ...
    charge_window_masks, export_window_masks = predictor.prepare_window_masks(
        charge_windows, export_windows, num_scenarios=4
    )
    ...
    print("\n" + "=" * 60)
    print("Best scenario: {} (£{:.2f})".format(
        ["ECO only", "Charge only", "Export only", "Both windows"][best_idx],
        results["final_cost"][best_idx]
    ))
    print("=" * 60)
