...
class Solcast:
    def __init__(self, time_format=None, time_format_solcast=None, time_format_forecast_solar=None, dp1_func=None, dp2_func=None, dp4_func=None, base=None):
        """
        Initialize Solcast component

        Args:
            time_format: Time format string for general timestamps
            time_format_solcast: Time format string for Solcast API
            time_format_forecast_solar: Time format string for Forecast.solar API
            dp1_func: Function for 1 decimal place formatting
            dp2_func: Function for 2 decimal place formatting
            dp4_func: Function for 4 decimal place formatting
            base: Base PredBat object for accessing shared functionality
        """
        # Accept base parameter for component pattern
        if base is not None:
            self.base = base
            self.log = base.log
            ...
            # Use time formats from base if not provided
            self.time_format = time_format or TIME_FORMAT
            self.time_format_solcast = time_format_solcast or TIME_FORMAT_SOLCAST
            self.time_format_forecast_solar = time_format_forecast_solar or TIME_FORMAT_FORECAST_SOLAR
            # Use utility functions from base if not provided
            self.dp1 = dp1_func or dp1
            self.dp2 = dp2_func or dp2
            self.dp4 = dp4_func or dp4
        else:
    ...
    def config_root(self):
        """Get config root from base or use default"""
        if hasattr(self, "base"):
            return self.base.config_root
        return getattr(self, "_config_root", "/config")

    @config_root.setter
    def config_root(self, value):
        """Set config root for backward compatibility"""
        self._config_root = value

    @property
    def config_root_p(self):
        """Get pretty config root from base or use default"""
        if hasattr(self, "base"):
            return self.base.config_root_p
        return getattr(self, "_config_root_p", "/config")

    @config_root_p.setter
    def config_root_p(self, value):
        """Set pretty config root for backward compatibility"""
        self._config_root_p = value
        ...
        for day in range(4):  # Today, tomorrow, d+2, d+3
            ...
            for minute in range(start_minute, end_minute, 30):
                ...
                if pv_estimate > 0 or pv_estimate10 > 0:  # Only include periods with data
                    ...
                    entry = {"period_start": period_start.strftime(self.time_format), "pv_estimate": pv_estimate, "pv_estimate10": pv_estimate10, "pv_estimate90": pv_estimate}  # Fallback to main estimate
        ...
        if pv_forecast_data:
            for entry in pv_forecast_data:
                ...
                if isinstance(period_start, str):
                    try:
                        period_start = datetime.strptime(period_start, self.time_format)
                    except (ValueError, TypeError):
                        continue
