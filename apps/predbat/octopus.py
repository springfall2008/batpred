...
class Octopus:
    ...
    def rate_add_io_slots(self, car_n, rates, octopus_slots):
        ...
        # "started" is the only level that leans on a live sensor rather than Octopus's own records,
        # so it needs a real car_charging_now for this car - a distinct charger/car-reported sensor
        # (docs/car-charging.md), not a load-signature guess that could take a kettle for the car
        # (#4885 review). Degrades per car, since a multi-car install may have it for only some.
        # fetch_sensor_data_cars() warns once at startup; this is the gate itself, re-evaluated each
        # cycle. Both ask has_car_charging_now_sensor(), so they cannot disagree.
        if trust_level == "started" and not self.has_car_charging_now_sensor(car_n):
            trust_level = "completed"
            ...
            for slot in octopus_slots:
                ...
                if self.dispatch_billed_off_peak(source, location, end_minutes):
                    ...
                    for minute in range(start_minutes, end_minutes):
                        ...
                        #
                        # The ladder, weakest evidence last ("started" already degraded to "completed"
                        # above for a car with no car_charging_now sensor):
                        #
                        #   none      - never trust a dynamic slot
                        #   completed - trust Octopus's own completed_dispatches record
                        #   started   - also trust a slot car_charging_now corroborated, without
                        #               waiting for that record (see get_car_charging_planned())
                        ...
                        # (test37_completed_does_not_trust_by_clock_time_alone). So `trusted`, unlike
